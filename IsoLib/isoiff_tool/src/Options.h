...
/*!
 ...
 @constant ISOIFF_ToolMode_Write Reading HEVC Bitstream and creating ISO Media Based Image File
 Format
 @constant ISOIFF_ToolMode_Read Reading ISO Media Based Image File Format and creating HEVC
 Bitstream
...
static inline void printUsage()
{
  ...
  printf("\t-m\t\tMode (mandatory) (0 = read bitstream, write file format; 1 = read file format, "
         "write bitstream)\n");
  ...
  printf("\t-o\t\tOutputfile (mandatory)\n");
  printf("\t-d\t\tDebuglevel. 0 = No Logging, 1 = Error, 2 = Warning, 3 = Info (Default), 4 = "
         "Debug, 5 = Trace\n");
  printf("\nExample:\n");
...
static inline void printOptions(Options *options)
{
  if(options->mode == 0)
  {
    logMsg(LOGLEVEL_INFO,
           "Mode:(Write) Reading HEVC Bitstream and creating ISO Media Based Image File Format");
    logMsg(LOGLEVEL_INFO, "Mode:(Write) Image dimension: %dx%d", options->width, options->height);
  }
  else if(options->mode == 1)
  {
    logMsg(LOGLEVEL_INFO,
           "Mode: (Read) Reading ISO Media Based Image File Format and creating HEVC Bitstream");
  }
...
static inline bool parseArguments(int argc, char **argv, Options *options)
{
  ...
  for(int i = 1; i < argc; ++i)
  {
    ...
    else if(!strcmp(argv[i], "-d"))
    {
      ...
      if((options->debugLevel < LOGLEVEL_OFF) && (options->debugLevel > LOGLEVEL_TRACE))
      {
        logMsg(LOGLEVEL_ERROR,
               "Option -d (Debuglevel): %s is not a valid debug level (must be [0 - 5])",
               argv[i + 1]);
        printUsage();
        return false;
      }
    ...
    else if(!strcmp(argv[i], "-m"))
    {
      ...
      {
        logMsg(LOGLEVEL_ERROR, "Option -m (Mode): %s is not a valid mode (must be [0, 1])",
               argv[i + 1]);
        printUsage();
        return false;
      }
    ...
    else if(!strcmp(argv[i], "-t"))
    {
      ...
      if((strcmp(options->inputType, "hevc") != 0) && (strcmp(options->inputType, "avc") != 0) &&
         (strcmp(options->inputType, "jpeg") != 0))
      {
        logMsg(LOGLEVEL_ERROR,
               "Option -t (InputType): %s is not a valid type (must be 'hevc', 'avc' or 'jpeg'])",
               argv[i + 1]);
        printUsage();
        return false;
      }
  ...
  {
    logMsg(LOGLEVEL_ERROR,
           "Option '-s width height' is mandatory for write mode (m = 0) execution.");
    printUsage();
    return false;
  }
