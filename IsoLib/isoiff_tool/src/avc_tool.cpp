	...
	while (!!bitstreamFile)
	{
		...
		if (!nalu.getBitstream().getFifo().empty())
		{
			...
			if (naluType == AVC_NALU_TYPE_SPS)
			{
				...
				err = avcC->addParameterSet(avcC, nalData, AVCsps);    if (err) goto bail;
			}
			else if (naluType == AVC_NALU_TYPE_PPS)
			{
				err = avcC->addParameterSet(avcC, nalData, AVCpps);    if (err) goto bail;
			}
			else if (naluType == AVC_NALU_TYPE_SPS_EXT)
			{
				err = avcC->addParameterSet(avcC, nalData, AVCspsext);    if (err) goto bail;
			}
			else
			{
...
MP4Err     addAVCImageToCollection(ISOIFF_ImageCollection collection, ISOVCConfigAtomPtr avcC, ISOIFF_HEVCItemData itemData, u32 width, u32 height)
{
	...
	err = ISOIFF_NewImage(collection, &image, ISOIFF_4CC_avc1, imageData);      if (err) goto bail;

	err = ISOIFF_AddImageProperty(image, (MP4AtomPtr)avcC, 1);					if (err) goto bail;
...
MP4Err     getAVCImages(ISOIFF_ImageCollection collection, ISOIFF_Image **images, ISOVCConfigAtomPtr **avcCAtoms, u32 *numberOfImagesFound)
{
	...
	err = MP4NoErr;
	err = ISOIFF_GetAllImagesWithType(collection, ISOIFF_4CC_avc1, images, numberOfImagesFound);            if (err) goto bail;
...
MP4Err     getAVCBitstreamFromImage(ISOIFF_Image image, ISOVCConfigAtomPtr avcC, MP4Handle bitstreamH)
{
	...
	{
		err = MP4GetListEntry(avcC->spsList, 0, (char **)&naluH);   if (err) goto bail;
	...
	{
		err = MP4GetListEntry(avcC->ppsList, 0, (char **)&naluH);   if (err) goto bail;
		err = MP4HandleCat(bitstreamH, zeroByteH);                  if (err) goto bail;
		err = MP4HandleCat(bitstreamH, startCodePrefixH);           if (err) goto bail;
		err = MP4HandleCat(bitstreamH, naluH);                      if (err) goto bail;
	}
	...
	{
		err = MP4GetListEntry(avcC->spsextList, 0, (char **)&naluH);   if (err) goto bail;
		err = MP4HandleCat(bitstreamH, zeroByteH);                  if (err) goto bail;
		err = MP4HandleCat(bitstreamH, startCodePrefixH);           if (err) goto bail;
		err = MP4HandleCat(bitstreamH, naluH);                      if (err) goto bail;
	}
	...
	err = ISOIFF_CreateHEVCItemDataFromHandle(imgDatH, &hevcItemData, avcC->length_size - 1);   if (err) goto bail;
