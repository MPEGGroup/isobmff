...
static MP4Err serialize( struct MP4Atom* s, char* buffer )
{
	...
	err = MP4GetHandleSize(self->jpegPrefix, &configSize); if (err) goto bail;
...
static MP4Err createFromInputStream( MP4AtomPtr s, MP4AtomPtr proto, MP4InputStreamPtr inputStream )
{
	...
	err = MP4NewHandle(bytesToRead, &self->jpegPrefix); if (err) goto bail;
...
MP4Err ISOIFF_CreateJPEGConfigurationAtom( ISOIFF_JPEGConfigurationAtomPtr *outAtom, MP4Handle jpegPrefix)
{
	MP4Err err;
	...
	err = MP4NewHandle(0, &self->jpegPrefix);								if (err) goto bail;
	err = MP4HandleCat(self->jpegPrefix, jpegPrefix);						if (err) goto bail;
