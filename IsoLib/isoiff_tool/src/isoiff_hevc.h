 ...
 * @param hevcItemData The HEVC item data
 * @param itemDataHandle Will contain the HEVC item data in bytestream form (Must be created with
 * MP4NewHandle before calling this function)
 */
MP4Err ISOIFF_PutHEVCItemDataIntoHandle(ISOIFF_HEVCItemData hevcItemData, MP4Handle itemDataHandle);

/*!
 * @discussion Creates a HEVC item data from a MP4Handle (This is the counterpart to
 ...
 * @param lengthSizeMinusOne This is needed to determine the size of the lenght field. (Can be found
 * in the HEVC decoder configuration record)
 */
MP4Err ISOIFF_CreateHEVCItemDataFromHandle(MP4Handle itemDataHandle,
 ...
 */
MP4Err ISOIFF_GetNALUDataHandlesFromHEVCItemData(ISOIFF_HEVCItemData hevcItemData,
                                                 MP4LinkedList naluDataHandles);

/*!
 * @discussion Gets the lengthSizeMinusOne (This is used for setting the  HEVC decoder configuration
 * record's field to the correct value)
 ...
 */
MP4Err ISOIFF_GetHEVCItemDataLengthSizeMinusOne(ISOIFF_HEVCItemData hevcItemData,
                                                u8 *lengthSizeMinusOne);
...
/*!
 * @discussion Adds a NAL unit to a HEVC decoder configuration record (VPS, SPS, PPS or SEI NAL
 * unit)
 ...
 * @param recordDataHandle Will contain the HEVC decoder configuration record in bytestream form
 * (Must be created with MP4NewHandle before calling this function)
 */
MP4Err ISOIFF_PutHEVCDecConfRecordIntoHandle(ISOIFF_HEVCDecoderConfigRecord hevcDecConfRec,
                                             MP4Handle recordDataHandle);

/*!
 * @discussion Creates a HEVC decoder configuration record from a MP4Handle (This is the counterpart
 * to ISOIFF_PutHEVCItemDataIntoHandle)
...
/*!
 * @discussion Collects all HEVC NAL units of a certain type from a HEVC decoder configuration
 * record (VPS, SPS, PPS or SEI NAL unit)
