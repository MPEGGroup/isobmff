...
MP4Err ISOIFF_CreateHEVCItemData(ISOIFF_HEVCItemData *hevcItemData)
{
  ...
  *hevcItemData = calloc(1, sizeof(struct ISOIFF_HEVCItemDataS));
  err           = MP4MakeLinkedList(&(*hevcItemData)->nalUnits);
  if(err) goto bail;
...
MP4Err ISOIFF_AddNALUnitToHEVCItemData(ISOIFF_HEVCItemData hevcItemData, MP4Handle nalUnitData)
{
  ...
  if(err) goto bail;
  err = MP4AddListEntry(hevcNalUnit, hevcItemData->nalUnits);
  if(err) goto bail;

  logMsg(LOGLEVEL_TRACE, "HEVC NAL unit size: %d", hevcNalUnit->nalUnitLength);

  if(hevcItemData->lengthInSizeMinusOne == 0)
    if((hevcNalUnit->nalUnitLength >> 8) > 0) hevcItemData->lengthInSizeMinusOne = 1;

  if(hevcItemData->lengthInSizeMinusOne == 1)
    if((hevcNalUnit->nalUnitLength >> 16) > 0) hevcItemData->lengthInSizeMinusOne = 3;
...
MP4Err ISOIFF_GetHEVCItemDataSize(ISOIFF_HEVCItemData hevcItemData, u32 *outSize)
{
  ...
  {
    ISOIFF_HEVCItemDataNalUnit hevcNalUnit;
    err = MP4GetListEntry(hevcItemData->nalUnits, i, (char **)&hevcNalUnit);
    if(err) goto bail;
...
MP4Err ISOIFF_PutHEVCItemDataIntoHandle(ISOIFF_HEVCItemData hevcItemData, MP4Handle itemDataHandle)
{
  ...
  bytePosition = 0;
  err          = ISOIFF_GetHEVCItemDataSize(hevcItemData, &size);
  if(err) goto bail;
  err = MP4SetHandleSize(itemDataHandle, size);
  if(err) goto bail;
                                           ...
                                           u8 lengthSizeMinusOne)
{
  ...
  bufferPosition = 0;
  buffer         = (u8 *)*itemDataHandle;
  err            = MP4GetHandleSize(itemDataHandle, &bufferSize);
  if(err) goto bail;
  err = ISOIFF_CreateHEVCItemData(outHevcItemData);
  if(err) goto bail;
  ...
  while(bufferPosition < bufferSize)
  {
    ...
    err = MP4AddListEntry(hevcNalUnit, (*outHevcItemData)->nalUnits);
    if(err) goto bail;
  }
bail:
  return err;
                                                 ...
                                                 MP4LinkedList naluDataHandles)
{
  ...
  for(i = 0; i < hevcItemData->nalUnits->entryCount; i++)
  {
    ...
    err = MP4GetListEntry(hevcItemData->nalUnits, i, (char **)&hevcNalUnit);
    if(err) goto bail;
    err = MP4AddListEntry(hevcNalUnit->nalUnitH, naluDataHandles);
    if(err) goto bail;
  }
...
MP4Err ISOIFF_FreeHEVCItemData(ISOIFF_HEVCItemData hevcItemData)
{
  ...
  err = MP4DeleteLinkedList(hevcItemData->nalUnits);
  if(err) goto bail;
