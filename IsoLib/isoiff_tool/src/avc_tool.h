/* This software module was originally developed by Apple Computer, Inc. in the course of
 ...
 company, the subsequent editors and their companies, and ISO/IEC have no liability for use of this
 software module or modifications thereof in an implementation. Copyright is not released for non
 MPEG-4 conforming products. Apple Computer, Inc. retains full right to use the code for its own
 purpose, assign or donate the code to a third party and to inhibit third parties from using the
 code for non MPEG-4 conforming products This copyright notice must be included in all copies or
 ...
 * @discussion Will read bytes from the option's inputFile and will parse the NAL Units and creating
 contents of the ISOIFF_AVCDecoderConfigRecord and the ISOIFF_AVCItemData for a single AVC still
 image.
...
/*!
 * @discussion Will add an AVC image item to the provided collection. The decoder configuration
 record will be linked to the image. NOTE: Before calling this function, create the image collection
 and generate the record and itemData using processAVC_NALUnits
...
/*!
 * @discussion Collects all AVC Images from a image collection and provides the results in form of
