                       ...
                       MP4Handle data)
{
  ...
  *image = calloc(1, sizeof(struct ISOIFF_ImageS));
  ...
  if(err) goto bail;
  err = ISOSetItemInfoItemType((*image)->item, type, NULL);
  if(err) goto bail;
  ...
  if(collection->isPrimaryImageSet == 0)
  {
    ...
    err                           = ISOIFF_SetImageAsCover(*image);
    if(err) goto bail;
  }
...
MP4Err ISOIFF_NewMeta(ISOIFF_ImageCollection collection, ISOIFF_Meta *meta, u32 type,
                      MP4Handle data)
{
  MP4Err err;
  ...
  *meta = calloc(1, sizeof(struct ISOIFF_MetaS));
  err   = ISOAddMetaItem(collection->meta, &(*meta)->item, 0, 0);
  if(err) goto bail;
  err = ISOAddItemExtentUsingItemData((*meta)->item, data);
  if(err) goto bail;
  err = ISOSetItemInfo((*meta)->item, 0, "image meta", NULL, NULL);
  if(err) goto bail;
  err = ISOSetItemInfoItemType((*meta)->item, type, NULL);
  if(err) goto bail;
...
MP4Err ISOIFF_WriteCollectionToFile(ISOIFF_ImageCollection collection, const char *filename)
{
  ...
  err = MP4WriteMovieToFile(collection->moov, filename);
  if(err) goto bail;
bail:
  return err;
}
...
MP4Err ISOIFF_ReadCollectionFromFile(ISOIFF_ImageCollection *collection, const char *filename)
{
  ...
  *collection = calloc(1, sizeof(struct ISOIFF_ImageCollectionS));
  ...
  if(outHandleType != ISOIFF_IMAGE_META_HANDLER_TYPE) err = MP4BadDataErr;

bail:
  return err;
}

MP4Err ISOIFF_GetAllImagesWithType(ISOIFF_ImageCollection collection, u32 type,
                                   ISOIFF_Image **images, u32 *numberOfImagesFound)
{
  MP4Err err;
  ISOMetaItem *items;
  u32 i;
  ...
  err = ISOGetAllItemsWithType(collection->meta, type, &items, numberOfImagesFound);
  if(err) goto bail;
...
MP4Err ISOIFF_GetAllMetasWithType(ISOIFF_ImageCollection collection, u32 type, ISOIFF_Meta **metas,
                                  u32 *numberOfMetasFound)
{
  ...
  err = ISOGetAllItemsWithType(collection->meta, type, &items, numberOfMetasFound);
  if(err) goto bail;
...
MP4Err ISOIFF_SetImageAsCover(ISOIFF_Image image)
{
  ...
  err = ISOIFF_GetImageType(image, &type);
  if(err) goto bail;

  logType(LOGLEVEL_DEBUG, "Setting Image Collection Cover Image. Type", type);

  err = ISOSetPrimaryItem(image->collection->meta, image->item);
  if(err) goto bail;
bail:
  return err;
}
...
MP4Err ISOIFF_AddImageRelation(ISOIFF_Image fromImage, ISOIFF_Image toImage, u32 relationType)
{
  ...
  err = ISOGetItemID(toImage->item, &toItemID);
  if(err) goto bail;
  err = ISOAddItemReference(fromImage->item, relationType, toItemID, &outIndex);
  if(err) goto bail;

  err = ISOIFF_GetImageType(fromImage, &fromType);
  if(err) goto bail;
  err = ISOIFF_GetImageType(toImage, &toType);
  if(err) goto bail;
...
MP4Err ISOIFF_AddMetaToImage(ISOIFF_Image fromImage, ISOIFF_Meta toMeta, u32 relationType)
{
  ...
  err = ISOGetItemID(toMeta->item, &toItemID);
  if(err) goto bail;
  err = ISOAddItemReference(fromImage->item, relationType, toItemID, &outIndex);
  if(err) goto bail;
  ...
  err = ISOIFF_GetImageType(fromImage, &fromType);
  if(err) goto bail;
  err = ISOIFF_GetMetaType(toMeta, &toType);
  if(err) goto bail;
...
MP4Err ISOIFF_AddImageToMeta(ISOIFF_Meta fromMeta, ISOIFF_Image toImage, u32 relationType)
{
  ...
  err = ISOGetItemID(toImage->item, &toItemID);
  if(err) goto bail;
  err = ISOAddItemReference(fromMeta->item, relationType, toItemID, &outIndex);
  if(err) goto bail;

  err = ISOIFF_GetMetaType(fromMeta, &fromType);
  if(err) goto bail;
  err = ISOIFF_GetImageType(toImage, &toType);
  if(err) goto bail;
...
MP4Err ISOIFF_GetImageType(ISOIFF_Image image, u32 *outType)
{
  ...
  tmp = NULL;
  err = ISOGetItemInfoItemType(image->item, outType, &tmp);
  if(err) goto bail;
  if(tmp) free(tmp);
bail:
  return err;
...
MP4Err ISOIFF_GetMetaType(ISOIFF_Meta meta, u32 *outType)
{
  ...
  tmp = NULL;
  err = ISOGetItemInfoItemType(meta->item, outType, &tmp);
  if(err) goto bail;
  if(tmp) free(tmp);
bail:
  return err;
...
MP4Err ISOIFF_GetImagesOfImageWithType(ISOIFF_Image fromImage, u32 relationType,
                                       ISOIFF_Image **toImages, u32 *numberOfImagesFound)
{
  MP4Err err;
  MP4Handle toItemIDs;
  u16 referenceCount;
  ...
  if(err) goto bail;
  err = ISOGetItemReferences(fromImage->item, relationType, &referenceCount, toItemIDs);
  if(err) goto bail;
  ...
  *toImages = calloc(referenceCount, sizeof(ISOIFF_Image));
  for(i = 0; i < referenceCount; i++)
  {
    ...
    err      = ISOFindItemByID(fromImage->collection->meta, &item, (u16)toItemIDsArray[i]);
    if(err) goto bail;
    refImage->item       = item;
...
MP4Err ISOIFF_GetMetasOfImageWithType(ISOIFF_Image fromImage, u32 relationType,
                                      ISOIFF_Meta **toMetas, u32 *numberOfMetasFound)
{
  MP4Err err;
  MP4Handle toItemIDs;
  u16 referenceCount;
...
MP4Err ISOIFF_GetImagesOfMetaWithType(ISOIFF_Meta fromMeta, u32 relationType,
                                      ISOIFF_Image **toImages, u32 *numberOfImagesFound)
{
  MP4Err err;
  MP4Handle toItemIDs;
  ...
  err = MP4NewHandle(0, &toItemIDs);
  if(err) goto bail;
  err = ISOGetItemReferences(fromMeta->item, relationType, &referenceCount, toItemIDs);
  if(err) goto bail;
  ...
  *toImages = calloc(referenceCount, sizeof(ISOIFF_Image));
  for(i = 0; i < referenceCount; i++)
  {
    ...
    image = calloc(1, sizeof(struct ISOIFF_ImageS));
    err   = ISOFindItemByID(fromMeta->collection->meta, &item, (u16)toItemIDsArray[i]);
    if(err) goto bail;
    image->item       = item;
...
MP4Err ISOIFF_FreeImageCollection(ISOIFF_ImageCollection collection)
{
  ...
  err = MP4DisposeMovie(collection->moov);
  if(err) goto bail;
  free(collection);
bail:
  return err;
...
MP4Err ISOIFF_GetImageProperties(ISOIFF_Image image, MP4GenericAtom **properties,
                                 u32 *propertiesFound)
{
  MP4Err err;
  if(image == NULL) return MP4BadParamErr;
