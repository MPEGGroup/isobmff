                                ...
                                u32 height)
{
  ...
  err = ISOIFF_NewImage(collection, &image, ISOIFF_4CC_jpeg, itemData);
  if(err) goto bail;

  if(jpgC)
  {
    err = ISOIFF_AddImageProperty(image, (MP4AtomPtr)jpgC, 1);
    if(err) goto bail;
  }
                     ...
                     ISOIFF_JPEGConfigurationAtomPtr **jpgCAtoms, u32 *numberOfImagesFound)
{
  ...
  err = MP4NoErr;
  err = ISOIFF_GetAllImagesWithType(collection, ISOIFF_4CC_jpeg, images, numberOfImagesFound);
  if(err) goto bail;

  logMsg(LOGLEVEL_INFO, "Number of images found: %d", *numberOfImagesFound);

  *jpgCAtoms = (ISOIFF_JPEGConfigurationAtomPtr *)calloc(*numberOfImagesFound,
                                                         sizeof(ISOIFF_JPEGConfigurationAtomPtr));
  ...
  for(i = 0; i < *numberOfImagesFound; i++)
  {
    ...
    for(j = 0; j < numberOfPropertiesFound; j++)
    {
      ...
      if(property->type == ISOIFF_4CC_jpgC)
      {
        ...
        err        = MP4NewHandle(unkownAtom->dataSize, &jpegPrefix);
        if(err) goto bail;
        buffer = (u8 *)*jpegPrefix;
        memcpy(buffer, data, unkownAtom->dataSize);
        err = ISOIFF_CreateJPEGConfigurationAtom(&jpgC, jpegPrefix);
        if(err) goto bail;
                                 ...
                                 MP4Handle bitstreamH)
{
  ...
  if(jpgC != NULL)
  {
    err = MP4HandleCat(bitstreamH, jpgC->jpegPrefix);
    if(err) goto bail;
  }

  err = MP4HandleCat(bitstreamH, imgDatH);
  if(err) goto bail;
bail:
  return err;
}
...
MP4Err processJPEGBitstream(MP4Handle prefixData, MP4Handle itemData, Options *options)
{
  ...
  err = MP4SetHandleSize(prefixData, prefixLength);
  if(err) goto bail;
  u8 *prefixHbuffer;
  ...
  err = MP4SetHandleOffset(inputH, prefixLength);
  if(err) goto bail;
  err = MP4HandleCat(itemData, inputH);
  if(err) goto bail;
