...
static MP4Err writeToFile(struct MP4MediaDataAtom *self, FILE *fd)
{
  ...
  err = MP4NoErr;
  if(fd == NULL) BAILWITHERROR(MP4BadParamErr)
  err = calculateSize((MP4AtomPtr)self);
  if(err) goto bail;
  if(self->size > 0)
  {
    ...
    if(written != self->bytesWritten) BAILWITHERROR(MP4IOErr)
#ifdef FORCE_OFFSET
    sought = fseeko(fd, FORCE_OFFSET, SEEK_CUR);
    if(sought != 0) BAILWITHERROR(MP4IOErr);
...
static MP4Err createFromInputStream(MP4AtomPtr s, MP4AtomPtr proto, MP4InputStreamPtr inputStream)
{
  ...
  if(s->size == 1) bytesToRead = s->size64 - s->bytesRead;
  else
