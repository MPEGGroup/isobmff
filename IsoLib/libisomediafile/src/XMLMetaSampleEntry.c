...
static MP4Err readstring(char **b, MP4XMLMetaSampleEntryAtomPtr self, MP4InputStreamPtr inputStream,
                         char *fieldname)
{
  u32 byte;
  MP4Err err;
...
static MP4Err createFromInputStream(MP4AtomPtr s, MP4AtomPtr proto, MP4InputStreamPtr inputStream)
{
  ...
  {
    err = readstring(&(self->content_encoding), self, inputStream, "content_encoding");
    if(err) goto bail;
  }
  else
    self->content_encoding = NULL;
  if(self->size > self->bytesRead)
  {
    err = readstring(&(self->xml_namespace), self, inputStream, "xml_namespace");
    if(err) goto bail;
  }
  else
    self->xml_namespace = NULL;
  if(self->size > self->bytesRead)
  {
    err = readstring(&(self->schema_location), self, inputStream, "schema_location");
    if(err) goto bail;
  }
  else
    self->xml_namespace = NULL;
