...
static MP4Err addDescriptor(struct MP4DescriptorRecord *s, MP4DescriptorPtr desc)
{
  ...
  /* try to handle OCI */
  if((desc->tag >= MP4ISO_OCI_EXT_RANGE_BEGIN) && (desc->tag <= MP4ISO_OCI_EXT_RANGE_END))
  {
  ...
  case MP4DecoderConfigDescriptorTag:
    if(self->decoderConfig) BAILWITHERROR(MP4InvalidMediaErr)
    self->decoderConfig = desc;
    break;

  case MP4SLConfigDescriptorTag:
    if(self->slConfig) BAILWITHERROR(MP4InvalidMediaErr)
    self->slConfig = desc;
    break;

  case MP4IPI_DescriptorPointerTag:
    if(self->ipiPtr) BAILWITHERROR(MP4InvalidMediaErr)
    self->ipiPtr = desc;
    break;

  case MPM4QoS_DescriptorTag:
    if(self->qos) BAILWITHERROR(MP4InvalidMediaErr)
    self->qos = desc;
    break;

  case MP4LanguageDescriptorTag:
    err = MP4AddListEntry(desc, self->langDesc);
    if(err) goto bail;
    break;
  ...
  case MP4SupplContentIdentDescriptorTag:
    err = MP4AddListEntry(desc, self->IPIDataSet);
    if(err) goto bail;
    break;
...
static MP4Err calculateSize(struct MP4DescriptorRecord *s)
{
  ...
  self->size += 3;
  if(self->dependsOnES) self->size += 2;
  if(self->OCRESID) self->size += 2;
...
MP4Err MP4CreateES_Descriptor(u32 tag, u32 size, u32 bytesRead, MP4DescriptorPtr *outDesc)
{
  ...
  err                 = MP4MakeLinkedList(&self->IPIDataSet);
  if(err) goto bail;
  err = MP4MakeLinkedList(&self->langDesc);
  if(err) goto bail;
