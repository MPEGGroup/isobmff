...
static MP4Err getOffset(struct MP4DataEntryAtom *self, u64 *outOffset)
{
  ...
  err = MP4NoErr;
  if(outOffset == NULL) BAILWITHERROR(MP4BadParamErr);
  if(self->mdat)
  {
...
static MP4Err addSamples(struct MP4DataEntryAtom *self, MP4Handle sampleH)
{
  ...
  err = MP4NoErr;
  err = MP4GetHandleSize(sampleH, &size);
  if(err) goto bail;
  if(self->mdat)
  {
    MP4MediaDataAtomPtr mdat;
    mdat = (MP4MediaDataAtomPtr)self->mdat;
    err  = mdat->addData(mdat, sampleH);
    if(err) goto bail;
    self->offset = mdat->dataSize;
  }
  else
...
static MP4Err calculateSize(struct MP4Atom *s)
{
  ...
  if((self->flags & 1) == 0)
  {
    ...
    if(self->name) self->size += 1 + (u32)strlen(self->name);
  }
bail:
  TEST_RETURN(err);
...
static MP4Err createFromInputStream(MP4AtomPtr s, MP4AtomPtr proto, MP4InputStreamPtr inputStream)
{
  ...
  if(self->nameURN == NULL) BAILWITHERROR(MP4NoMemoryErr)
  if(bytesToRead)
  {
    ...
    if(self->location == NULL) BAILWITHERROR(MP4BadDataErr)
  }
  else
...
bail:
  ...
  if(err)
  {
    if(self->nameURN) free(self->nameURN);
  }
  return err;
}
