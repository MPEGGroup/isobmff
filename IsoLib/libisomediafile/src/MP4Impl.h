...
#define DEBUG_MSG(str) inputStream->msg(inputStream, str);
...
#define GETBYTES(length, membername)                                               \
  err = inputStream->readData(inputStream, length, self->membername, #membername); \
  if(err) goto bail;                                                               \
  self->bytesRead += length

#define GETBYTES_V(length, var)                                \
  err = inputStream->readData(inputStream, length, var, NULL); \
  if(err) goto bail;                                           \
  self->bytesRead += length
...
#define GETBYTES_V_MSG(length, var, msg)                      \
  err = inputStream->readData(inputStream, length, var, msg); \
  if(err) goto bail;                                          \
  self->bytesRead += length
...
#define GET64(membername)                                               \
  {                                                                     \
    u64 val64;                                                          \
    u32 val32;                                                          \
    err = inputStream->read32(inputStream, (u32 *)&val32, #membername); \
    if(err) goto bail;                                                  \
    val64 = val32;                                                      \
    val64 <<= 32;                                                       \
    err = inputStream->read32(inputStream, (u32 *)&val32, #membername); \
...
#define GET64_MSG(membername, msg)                              \
  {                                                             \
    u64 val64;                                                  \
    u32 val32;                                                  \
    err = inputStream->read32(inputStream, (u32 *)&val32, msg); \
    if(err) goto bail;                                          \
    val64 = val32;                                              \
    val64 <<= 32;                                               \
    err = inputStream->read32(inputStream, (u32 *)&val32, msg); \
...
#define GET64_V(varname)                                             \
  {                                                                  \
    u64 val64;                                                       \
    u32 val32;                                                       \
    err = inputStream->read32(inputStream, (u32 *)&val32, #varname); \
    if(err) goto bail;                                               \
    val64 = val32;                                                   \
    val64 <<= 32;                                                    \
    err = inputStream->read32(inputStream, (u32 *)&val32, #varname); \
...
#define GET32(membername)                                                        \
  err = inputStream->read32(inputStream, (u32 *)&self->membername, #membername); \
  if(err) goto bail;                                                             \
  self->bytesRead += 4

#define GET32_MSG(membername, msg)                                       \
  err = inputStream->read32(inputStream, (u32 *)&self->membername, msg); \
  if(err) goto bail;                                                     \
  self->bytesRead += 4

#define GET32_V(varname)                                             \
  err = inputStream->read32(inputStream, (u32 *)&varname, #varname); \
  if(err) goto bail;                                                 \
  self->bytesRead += 4
...
#define GET16(membername)                                                        \
  err = inputStream->read16(inputStream, (u32 *)&self->membername, #membername); \
  if(err) goto bail;                                                             \
  self->bytesRead += 2
...
#define GET16_V(varname)                                             \
  err = inputStream->read16(inputStream, (u32 *)&varname, #varname); \
  if(err) goto bail;                                                 \
  self->bytesRead += 2
...
#define GET8(membername)                                                        \
  err = inputStream->read8(inputStream, (u32 *)&self->membername, #membername); \
  if(err) goto bail;                                                            \
  self->bytesRead += 1
...
#define GET8_V_MSG(varname, msg)                               \
  err = inputStream->read8(inputStream, (u32 *)&varname, msg); \
  if(err) goto bail;                                           \
  self->bytesRead += 1

#define GETATOM(membername)                           \
  err = MP4ParseAtom(inputStream, &self->membername); \
  if(err) goto bail;                                  \
  self->bytesRead += self->membername->size;

#define GETATOM_V(varname)                   \
  err = MP4ParseAtom(inputStream, &varname); \
  if(err) goto bail;                         \
...
#define GETATOM_LIST(listname)                                                      \
  {                                                                                 \
    ...
    {                                                                               \
      ...
      {                                                                             \
        err = MP4AddListEntry((void *)atm, self->listname);                         \
        if(err) goto bail;                                                          \
      }                                                                             \
    }                                                                               \
  }
...
#define ADD_DESCRIPTOR_SIZE(descName)                    \
  if(self->descName)                                     \
  {                                                      \
    err = self->descName->calculateSize(self->descName); \
    if(err) goto bail;                                   \
...
#define ADD_DESCRIPTOR_LIST_SIZE(listName)                      \
  if(self->listName)                                            \
  {                                                             \
    err = MP4GetListEntryCount(self->listName, &count);         \
    if(err) goto bail;                                          \
    for(i = 0; i < count; i++)                                  \
    {                                                           \
      err = MP4GetListEntry(self->listName, i, (char **)&desc); \
      if(err) goto bail;                                        \
      if(desc)                                                  \
      {                                                         \
...
#define ADD_ATOM_LIST_SIZE_V(listName)                    \
  ...
  {                                                       \
    u32 count;                                            \
    u32 i;                                                \
    err = MP4GetListEntryCount(listName, &count);         \
    if(err) goto bail;                                    \
    for(i = 0; i < count; i++)                            \
    {                                                     \
      MP4AtomPtr desc;                                    \
      err = MP4GetListEntry(listName, i, (char **)&desc); \
      if(err) goto bail;                                  \
      if(desc)                                            \
      {                                                   \
        err = desc->calculateSize(desc);                  \
...
#define SERIALIZE_DESCRIPTOR(desc)                         \
  ...
  {                                                        \
    ...
    err = self->desc->serialize(self->desc, buffer);       \
    if(err) goto bail;                                     \
...
#define DESTROY_ATOM_LIST_V(atomList)                    \
  ...
  {                                                      \
    u32 atomListSize;                                    \
    err = MP4GetListEntryCount(atomList, &atomListSize); \
    if(err) goto bail;                                   \
    for(i = 0; i < atomListSize; i++)                    \
    {                                                    \
      MP4AtomPtr a;                                      \
      err = MP4GetListEntry(atomList, i, (char **)&a);   \
      if(err) goto bail;                                 \
      if(a) a->destroy(a);                               \
    }                                                    \
    err = MP4DeleteLinkedList(atomList);                 \
    if(err) goto bail;                                   \
  }
...
#define DESTROY_DESCRIPTOR(member) \
  if(self->member) self->member->destroy(self->member)
...
#define DESTROY_DESCRIPTOR_LIST(list)                    \
  ...
  {                                                      \
    ...
    u32 listSize;                                        \
    u32 i;                                               \
    err = MP4GetListEntryCount(self->list, &listSize);   \
    if(err) goto bail;                                   \
    for(i = 0; i < listSize; i++)                        \
    {                                                    \
      MP4DescriptorPtr a;                                \
      err = MP4GetListEntry(self->list, i, (char **)&a); \
      if(err) goto bail;                                 \
      if(a) a->destroy(a);                               \
    }                                                    \
    err = MP4DeleteLinkedList(self->list);               \
    if(err) goto bail;                                   \
  }
...
#define DESTROY_DESCRIPTOR_LIST_V(list)            \
  ...
  {                                                \
    ...
    u32 listSize;                                  \
    u32 i;                                         \
    err = MP4GetListEntryCount(list, &listSize);   \
    if(err) goto bail;                             \
    for(i = 0; i < listSize; i++)                  \
    {                                              \
      MP4DescriptorPtr a;                          \
      err = MP4GetListEntry(list, i, (char **)&a); \
      if(err) goto bail;                           \
      if(a) a->destroy(a);                         \
    }                                              \
    err = MP4DeleteLinkedList(list);               \
    if(err) goto bail;                             \
  }
