...
MP4Err MP4ParseIPMPXData(MP4InputStreamPtr inputStream, MP4IPMPXDataPtr *outData)
{
  ...
  err = MP4NoErr;
  if((inputStream == NULL) || (outData == NULL)) BAILWITHERROR(MP4BadParamErr)
  bytesRead = 0;
  ...
  default:
    err = MP4CreateIPMPXDefaultData(tag, size, bytesRead, &data);
    if(err) goto bail;
    break;
  }
  ...
  err = data->createFromInputStream(data, inputStream);
  if(err) goto bail;
  *outData = data;
bail:
  TEST_RETURN(err);
...
MP4Err MP4EncodeBaseIPMPXData(struct MP4IPMPXDataRecord *self, char *buffer)
{
  ...
  vals[3] = (u8)(length & 0x7f);
  length >>= 7;
  vals[2] = (u8)((length & 0x7f) | 0x80);
  length >>= 7;
  vals[1] = (u8)((length & 0x7f) | 0x80);
  length >>= 7;
