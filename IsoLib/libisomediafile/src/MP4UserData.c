                       ...
                       u32 itemIndex)
{
  ...
  err = udta->getAtom(udta, &atom, userDataType, itemIndex);
  if(err) goto bail;
...
MP4DeleteUserDataItem(MP4UserData theUserData, u32 userDataType, u32 itemIndex)
{
  ...
  err = udta->deleteItem(udta, userDataType, itemIndex);
  if(err) goto bail;
bail:
  TEST_RETURN(err);
...
MP4NewUserData(MP4UserData *outUserData)
{
  ...
  MP4AtomPtr udta;
  err = MP4NoErr;
  err = MP4CreateAtom(MP4UserDataAtomType, &udta);
  if(err) goto bail;
...
MP4NewForeignAtom(MP4GenericAtom *outAtom, u32 atomType, MP4Handle atomPayload)
{
  ...
  err = MP4NoErr;
  err = MP4CreateUnknownAtom((MP4UnknownAtomPtr *)&the_atom);
  if(err) goto bail;
  self       = (MP4UnknownAtomPtr)the_atom;
  self->type = atomType;

  err = MP4GetHandleSize(atomPayload, &(self->dataSize));
  if(err) goto bail;
...
MP4GetForeignAtom(MP4GenericAtom atom, u32 *atomType, u8 the_uuid[16], MP4Handle atomPayload)
{
  ...
  *atomType = self->type;
  ...
  err = MP4SetHandleSize(atomPayload, self->dataSize);
  if(err) goto bail;

  memcpy(*atomPayload, self->data, self->dataSize);
  if((self->type == MP4ExtendedAtomType) && the_uuid) memcpy(the_uuid, &(self->uuid), 16);
bail:
  TEST_RETURN(err);
...
MP4NewUUIDAtom(MP4GenericAtom *outAtom, u8 the_uuid[16], MP4Handle atomPayload)
{
  ...
  err = MP4NoErr;
  err = MP4CreateAtom(MP4ExtendedAtomType, &the_atom);
  if(err) goto bail;
  self       = (MP4UnknownAtomPtr)the_atom;
