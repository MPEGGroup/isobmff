...
static MP4Err setEntry(struct MP4SampleToChunkAtom *self, u32 firstChunkNumber, u32 sampleCount,
                       u32 sampleDescriptionIndex)
{
  stscEntryPtr p;
  stscEntryPtr last;
  ...
  if(entryCount > 0)
  {
    err = MP4GetListEntry(self->entryList, entryCount - 1, (char **)&last);
    if(err) goto bail;
    if((last->samplesPerChunk == sampleCount) &&
...
static MP4Err mdatMoved(struct MP4SampleToChunkAtom *self, u64 mdatBase, u64 mdatEnd,
                        s32 mdatOffset, struct MP4SampleDescriptionAtom *stsd,
  ...
  */
  ...
  for(i = 0; i < entryCount; i++)
  {
    ...
    if(p->sampleDescriptionIndex != savedDescIndex)
    {
      ...
      err            = stsd->getEntry(stsd, p->sampleDescriptionIndex, &sampleEntry);
      if(err) goto bail;
      /* get data entry */
    ...
    if(savedSelfContained)
    {
      ...
      if(i == (entryCount - 1)) chunks = chunkCount + 1 - p->firstChunk;
      else
      {
        stscEntryPtr q;
        err = MP4GetListEntry(self->entryList, i + 1, (char **)&q);
        if(err) goto bail;
      ...
      */
      err = stco->mdatMoved(stco, p->firstChunk, p->firstChunk + chunks - 1, mdatBase, mdatEnd,
                            mdatOffset);
      if(err) goto bail;
    }
  ...
  *outSampleDescriptionIndex = 0;
  for(; i < entryCount; i++)
  {
    ...
    else
    {
      ...
      err = MP4GetListEntry(self->entryList, i + 1, (char **)&nextEntryP);
      if(err) goto bail;
...
static MP4Err createFromInputStream(MP4AtomPtr s, MP4AtomPtr proto, MP4InputStreamPtr inputStream)
{
  ...
  for(i = 0; i < entryCount; i++)
  {
    ...
    err = setEntry(self, firstChunk, samplesPerChunk, sampleDescriptionIndex);
    if(err) goto bail;
  }
bail:
  TEST_RETURN(err);
