...
MP4Err MP4ParseCommand(MP4InputStreamPtr inputStream, MP4DescriptorPtr *outDesc)
{
  ...
  case MP4ObjectDescriptorUpdateTag:
    err = MP4CreateObjectDescriptorUpdate(tag, size, bytesRead, &desc);
    if(err) goto bail;
    break;

  case MP4ESDescriptorUpdateTag:
    err = MP4CreateESDescriptorUpdate(tag, size, bytesRead, &desc);
    if(err) goto bail;
    break;

  case MP4IPMP_ToolDescriptorUpdateTag:
    err = MP4CreateIPMPToolDescriptorUpdate(tag, size, bytesRead, &desc);
    if(err) goto bail;
    break;

  default:
    err = MP4CreateDefaultCommand(tag, size, bytesRead, &desc);
    if(err) goto bail;
    break;
  }
