...
static MP4Err createFromInputStream( MP4AtomPtr s, MP4AtomPtr proto, MP4InputStreamPtr inputStream )
{
    ...
    if (flags & trun_data_offset_present)				{ GET32( data_offset ); field_count--; };
    if (flags & trun_first_sample_flags_present) 		{ GET32( first_sample_flags ); field_count--; };
	while (field_count > 0) { GET32_V(junk); field_count--; }
	
    for (i=0, entry = self->entries; i<self->samplecount; i++, entry++) {
		r = record_count;
	    if (flags & trun_sample_duration_present)
			{ GET32_V_MSG(( entry->sample_duration ), "duration"); r--; };    
	    if (flags & trun_sample_size_present)
			{ GET32_V_MSG(( entry->sample_size ), "size"); r--; };
	    if (flags & trun_sample_flags_present)
			{ GET32_V_MSG(( entry->sample_flags ), "flags"); r--; };
	    if (flags & trun_sample_composition_times_present)
			{ GET32_V_MSG(( entry->sample_composition_time_offset ), "comp offset"); r--; };
		while (r > 0) { GET32_V(junk); r--; }
    }
