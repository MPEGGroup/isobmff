...
static MP4Err addSamples(struct MP4TimeToSampleAtom *self, u32 sampleCount, MP4Handle durationsH)
{
  ...
  err       = MP4GetHandleSize(durationsH, &hasDurations);
  if(err) goto bail;
...
static MP4Err extendLastSampleDuration(struct MP4TimeToSampleAtom *self, u32 duration)
{
  ...
  {
    current->sampleCount--;
    err = addSample(self, current->sampleDuration + duration);
    if(err) goto bail;
  }
                                     ...
                                     s32 *outSampleDuration)
{
  ...
  if(outSampleCTS) *outSampleCTS = sampleCTS;
  if(outSampleDuration) *outSampleDuration = sampleDuration;
  self->foundEntry             = pe;
...
static MP4Err createFromInputStream(MP4AtomPtr s, MP4AtomPtr proto, MP4InputStreamPtr inputStream)
{
  ...
  for(i = 0; i < entryCount; i++)
  {
    ...
    err                = MP4AddListEntry(pe, self->entryList);
    if(err) goto bail;
