...
static MP4Err addSamples(struct MP4SampleSizeAtom *self, u32 sampleCount, MP4Handle sizesH)
{
  ...
  err         = MP4GetHandleSize(sizesH, &sizesCount);
  if(err) goto bail;
  ...
  if(varyingsizes || (*sampleSizes == 0) ||
     ((self->sampleCount > 0) && (*sampleSizes != self->sampleSize)))
  {
    u32 newsize;
    ...
    {
      while(self->allocatedSize < newsize)
...
static MP4Err setfieldsize(MP4SampleSizeAtomPtr self, u32 fieldsize)
{
  ...
  if((self->fieldsize == 32) || (self->sampleSize != 0)) self->type = MP4SampleSizeAtomType;
  else
...
static MP4Err calculateSize(struct MP4Atom *s)
{
  ...
  if(self->sampleSize == 0) switch(self->fieldsize)
    {
    case 4:
      self->size += ((self->sampleCount + 1) / 2);
      break;
    case 8:
      self->size += (self->sampleCount);
      break;
    case 16:
      self->size += (2 * self->sampleCount);
      break;
    case 32:
