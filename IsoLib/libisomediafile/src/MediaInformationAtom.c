...
static MP4Err setSampleDependency( struct MP4MediaInformationAtom *self, s32 sample_index, MP4Handle dependencies  )
{
	MP4Err err;
	MP4SampleTableAtomPtr stbl;
   ...
   err = stbl->setSampleDependency( stbl, sample_index, dependencies ); if (err) goto bail;
  bail:
   TEST_RETURN( err );
...
static MP4Err getSampleDependency( struct MP4MediaInformationAtom *self, u32 sampleNumber, u8* dependency  )
{
	MP4Err err;
	MP4SampleTableAtomPtr stbl;
    ...
    err = stbl->getSampleDependency( stbl, sampleNumber, dependency ); if (err) goto bail;
  bail:
   TEST_RETURN( err );
