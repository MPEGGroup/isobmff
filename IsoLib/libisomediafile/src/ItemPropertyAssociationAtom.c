...
static void destroy( MP4AtomPtr s )
{
	...
	err = MP4GetListEntryCount(self->entries, &entry_count); if (err) goto bail;

	for (i = 0; i < entry_count; i++)
	{
		err = MP4GetListEntry(self->entries, i, (char **)&entry); if (err) goto bail;
		...
		{
			err = MP4GetListEntry(entry->propertyIndexes, j, (char **)&propertyIndex); if (err) goto bail;
			free(propertyIndex);
		}

		err = MP4DeleteLinkedList(entry->propertyIndexes); if (err) goto bail;
		free(entry);
	}

	err = MP4DeleteLinkedList(self->entries); if (err) goto bail;
...
static MP4Err createFromInputStream( MP4AtomPtr s, MP4AtomPtr proto, MP4InputStreamPtr inputStream )
{
	...
	for (i = 0; i < entry_count; i++)
	{
		...
		TESTMALLOC(entry);
		err = MP4AddListEntry(entry, self->entries); if (err) goto bail;
		err = MP4MakeLinkedList(&entry->propertyIndexes); if (err) goto bail;
		...
		for (j = 0; j < association_count; j++)
		{
			...
			err = MP4AddListEntry(propertyIndex, entry->propertyIndexes); if (err) goto bail;
			...
			else
			{
				...
				if (propertyIndex->property_index & (1 << 7))
					propertyIndex->essential = 1;
...
static MP4Err addEntry(MP4ItemPropertyAssociationAtomPtr self, MP4ItemPropertyAssociationEntryPtr entry)
{
	MP4Err err;
	err = MP4NoErr;
...
MP4Err MP4CreateItemPropertyAssociationAtom( MP4ItemPropertyAssociationAtomPtr *outAtom )
{
	...
	err = MP4MakeLinkedList(&self->entries); if (err) goto bail;
