...
typedef struct {
 ...
 1. Run through sample group set: i=[0,n]
 	...
 	b.  Initialize memory costs in tracker assuming each sample is a distinct non-pattern C[i] = (i*indexFieldSize)
 2. Run through sample group set: i=[0,n]
 	a. Update memory cost considering sample i is a distinct non-pattern C[i] = min(C[i] , C[i-1]+ indexFieldSize)
 	b. For pattern lengths, p=[1, 32]
		...
		ii. Extend right end from [i+1, n] in the look ahead buffer
				1. As long as the pattern repeats, keep extending right end and update memory cost
  					C[end] = min(C[end] , C[start-1] + p*indexFieldSize
				2. If C[end] got modified, a more efficient pattern was found, maintain previousEfficientIndex = start-1 for backtracking.
 3. Tracker[n-1] represents most efficienct way to encode samples [0,n]. Run through tracker backwards from last sample: i[n-1, 0].
  	a. Use previousEfficientIndex to populate nextEfficientIndex to easily create compact sample groups
	b. Patterns with (pattern length == sample count == 1), are combined and considered as non-pattern
  
Algorithm can further be optimized by making memory cost consider patternLength and sampleCount field sizes
 */
...
u8 GetFieldSize(u32 value, u8 localFragmentUsed)
{
...
void  AppendNewPatternEntry(CompressedGroupInfo* compressedGroup, u32 index, u32 patternLength, u32 sampleCount)
{
...
void SetMemoryCostForPattern(SampleGroupInfo *sampleGroup, u32 patternLength, u32 startPatternSampleIndex, u32 endPatternSampleIndex)
{
	...
	s32 memoryCost = startPatternSampleIndex <= 0 ? sampleGroup->patternTracker[0].cumulativeMemoryCost : sampleGroup->patternTracker[startPatternSampleIndex - 1].cumulativeMemoryCost;
...
void SetMemoryCostForNonPattern(SampleGroupInfo *sampleGroup, s32 sampleIndex)
{
	s32 memoryCost = (sampleIndex < 1 ? sampleGroup->patternTracker[0].cumulativeMemoryCost: sampleGroup->patternTracker[sampleIndex-1].cumulativeMemoryCost) + sampleGroup->groupIndexFieldSize;
...
void CombineNonPatterns(SampleGroupInfo *sampleGroup, CompressedGroupInfo* compressedGroup)
{
	...
	while(prevIndex >= 0 && prevIndex < sampleGroup->sampleCount)
	{
		...
		s32 i = prevIndex;
		while(i >= 0 && i < sampleGroup->sampleCount && (patternTracker[i].patternLength == 1) && (patternTracker[i].sampleCount == 1))
		{
...
void FindPatternsStartingAtIndex(SampleGroupInfo *sampleGroup, u32 sampleIndex)
{
	...
	for (u32 p = 1; p <= kMaxPatternLength; p++)
	{
		...
		if (sampleGroup->groupIndex[sampleIndex] == sampleGroup->groupIndex[sampleIndex - p]) {
			...
			u32 i = sampleIndex;
			while (i < sampleGroup->sampleCount && sampleGroup->groupIndex[i] == sampleGroup->groupIndex[i - p]) {
...
void InitializeSampleGroupInput(MP4CompactSampletoGroupAtomPtr self, SampleGroupInfo* sampleGroup)
{
	...
	sampleGroup->patternTracker 		= (PatternTracker*) malloc(sizeof(PatternTracker)*self->sampleCount);
...
void CreateCompactSampleGroups(MP4CompactSampletoGroupAtomPtr self)
{
	...
	CompressedGroupInfo* compressedGroup = &self->compressedGroup;
	...
	if(compressedGroup->patternEntries == NULL) {
		compressedGroup->patternEntries = (MP4CompactSampleToGroupPatternEntryPtr) malloc(sizeof(MP4CompactSampleToGroupPatternEntry)*compressedGroup->patternCount);
	}
	if(compressedGroup->indexDescriptionArray == NULL) {
		compressedGroup->indexDescriptionArray = (u32*) malloc(sizeof(u32)*compressedGroup->totalIndexDescriptionCount);
	}
	
	/*printf("\n The pattern tracker details are");
	for(int i = 0; i < self->sampleCount; i++)
	{
		printf("\n Index %d Start: %d  PL: %d  SC: %d  Eff: %d  PrevIndex: %d NextIndex: %d ", i, patternTracker[i].start, patternTracker[i].patternLength, patternTracker[i].sampleCount, patternTracker[i].cumulativeMemoryCost, patternTracker[i].prevEfficientIndex, patternTracker[i].nextEfficientIndex);
	}*/
	...
	for(u32 patternIndex = 0; patternIndex < compressedGroup->patternCount; patternIndex++) {
		...
		u32 sampleCount 	= patternTracker[nextIndex].sampleCount;
...
{
	MP4Err err;
	MP4CompactSampletoGroupAtomPtr self;
	err = MP4NoErr;
	self = (MP4CompactSampletoGroupAtomPtr) s;
...
static MP4Err ensureSize( struct MP4CompactSampletoGroupAtom *self, u32 newSize )
{
	MP4Err err;
...
static MP4Err addSamples( struct MP4CompactSampletoGroupAtom *self, u32 count )
{
	MP4Err err;
	u32 *p;
	u32 j;
...
static MP4Err mapSamplestoGroup( struct MP4CompactSampletoGroupAtom *self, u32 group_index, s32 sample_index, u32 count  )
{
	MP4Err err;
	u32 i;
	u32* p;
	...
	if(GetFieldSize(group_index, self->fragmentLocalIndexPresent) > self->compressedGroup.indexFieldSize)
		self->compressedGroup.indexFieldSize = GetFieldSize(group_index, self->fragmentLocalIndexPresent);
...
static MP4Err getSampleGroupMap( struct MP4CompactSampletoGroupAtom *self, u32 sampleNumber, u32* groupIndex  )
{
	MP4Err err;
...
static MP4Err GetData(MP4AtomPtr s, MP4InputStreamPtr inputStream, u8 fieldSize, u32* upperNibble, u32* outValue)
{
	MP4Err err = MP4NoErr;
	MP4CompactSampletoGroupAtomPtr self = (MP4CompactSampletoGroupAtomPtr) s;
	
	if(fieldSize != 4 && fieldSize != 8 && fieldSize != 16 && fieldSize != 32) BAILWITHERROR( MP4BadParamErr )
...
static MP4Err serialize( struct MP4Atom* s, char* buffer )
{
	...
	self->flags = SetFieldSize(self->compressedGroup.indexFieldSize) | (SetFieldSize(self->compressedGroup.sampleCountFieldSize) << 2) | (SetFieldSize(self->compressedGroup.patternLengthFieldSize) << 4) | (self->fragmentLocalIndexPresent ? 0x80 : 0);
...
static MP4Err calculateSize( struct MP4Atom* s )
{
	...
	sizeInBits += (self->compressedGroup.totalIndexDescriptionCount*self->compressedGroup.indexFieldSize);
...
static MP4Err createFromInputStream( MP4AtomPtr s, MP4AtomPtr proto, MP4InputStreamPtr inputStream )
{
	...
	if(indexFieldSize != 4 && indexFieldSize != 8 && indexFieldSize != 16 && indexFieldSize != 32) BAILWITHERROR( MP4BadParamErr )
	
	countFieldSize			= 4 << ((flags & 0xC) >> 2);
	if(countFieldSize != 4 && countFieldSize != 8 && countFieldSize != 16 && countFieldSize != 32) BAILWITHERROR( MP4BadParamErr )
	...
	sprintf( msgString, "field size for index %d , sample count %d pattern length %d ", indexFieldSize, countFieldSize, patternFieldSize );
	...
	MP4CompactSampleToGroupPatternEntryPtr patternEntries = malloc(self->entryCount* sizeof(MP4CompactSampleToGroupPatternEntry));
	...
	for (i=0; i<self->entryCount; i++) {
		...
		sprintf( msgString, "pattern entry %d: pattern length->%d sample count->%d ", i+1, patternLength, sampleCount  );
		inputStream->msg( inputStream, msgString );
		
	}

	err = ensureSize( self, self->sampleCount*sizeof(u32) ); if (err) goto bail;
	...
	for (i=0; i<self->entryCount; i++) {
		u32* patternStart = p;
		u32 mask = self->fragmentLocalIndexPresent? 0xFFFFFFFF >> (32 - indexFieldSize + 1): 0xFFFFFFFF;
...
MP4Err MP4CreateCompactSampletoGroupAtom( MP4CompactSampletoGroupAtomPtr *outAtom )
{
	...
	self = (MP4CompactSampletoGroupAtomPtr) calloc( 1, sizeof(MP4CompactSampletoGroupAtom) );
	TESTMALLOC( self )
