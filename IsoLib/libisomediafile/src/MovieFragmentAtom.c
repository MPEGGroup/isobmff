...
static MP4Err mergeFragments( struct MP4MovieFragmentAtom* self, MP4MovieAtomPtr moov )
{
	...
	for ( i = 0; i < trackCount; i++ )
	{
		...
		if ((tfhd_flags & tfhd_base_data_offset_present)==0)
			tfhd->base_data_offset = (tfhd_flags & tfhd_default_base_is_moof ? 
										self->streamOffset : base_offset);
			 
		err = traf->calculateDataEnd( traf, &traf_data_end ); if (err) goto bail;
...
static MP4Err addAtom( MP4MovieFragmentAtomPtr self, MP4AtomPtr atom )
{
	...
	switch (atom->type) {
		...
		default: err = MP4AddListEntry( atom, self->atomList ); break;
	}
