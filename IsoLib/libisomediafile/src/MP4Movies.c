...
MP4_EXTERN(MP4Err) MP4DisposeMovie(MP4Movie theMovie)
{
  ...
  if(moov->inputStream) moov->inputStream->destroy(moov->inputStream);
  ...
  if(moov->mdat) moov->mdat->destroy(moov->mdat);

  if(moov->meta) moov->meta->destroy(moov->meta);

  if(moov->jp2h) moov->jp2h->destroy(moov->jp2h);

  if(moov->ftyp) moov->ftyp->destroy(moov->ftyp);

  if(moov->sgnt) moov->sgnt->destroy(moov->sgnt);

  if(moov->prepend_handle) MP4DisposeHandle(moov->prepend_handle);
...
static MP4Err parseMovie(MP4Movie *theMovie, MP4PrivateMovieRecordPtr moov, int openMovieFlags)
{
  ...
  err = MP4NoErr;
  if((moov == NULL) || (moov->inputStream == NULL)) BAILWITHERROR(MP4BadParamErr);
  ...
  for(finished = 0; !finished;)
  {
    ...
    case MP4MediaDataAtomType:
    {
      ...
      else
      {
        ...
        {
          err = MP4MakeLinkedList(&moov->mdatList);
          if(err) goto bail;
        }
        err = MP4AddListEntry(anAtomPtr, moov->mdatList);
        if(err) goto bail;
      }
    }
    break;
    case MP4MovieFragmentAtomType:
      ...
      {
        err = MP4MakeLinkedList(&moov->movieFragments);
        if(err) goto bail;
      }
      err = MP4AddListEntry(anAtomPtr, moov->movieFragments);
      if(err) goto bail;
      break;
    ...
    if(moov->inputStream->available == 0) finished = 1;
  }
  /* only mp4v1 files have no ftyp, and they have to have a moov */
  if((moov->moovAtomPtr == NULL) && (moov->ftyp == NULL)) BAILWITHERROR(MP4BadDataErr)

  err = MergeMovieFragments(moov);
  if(err) goto bail;

  if(moov->mdat != NULL)
  {
    err = MergeMovieData(moov);
    if(err) goto bail;
  }

  if(err == MP4NoErr) *theMovie = (MP4Movie)moov;
bail:
  TEST_RETURN(err);
...
MP4_EXTERN(MP4Err) MergeMovieFragments(MP4PrivateMovieRecordPtr movie)
{
  ...
  for(i = 0; i < fragmentCount; i++)
  {
    ...
    err = MP4GetListEntry(movie->movieFragments, i, (char **)&moof);
    if(err) goto bail;
    ...
    err = moof->mergeFragments(moof, (MP4MovieAtomPtr)(movie->moovAtomPtr));
    if(err) goto bail;
  }
...
MP4_EXTERN(MP4Err) MergeMovieData(MP4PrivateMovieRecordPtr movie)
{
  ...
  if(meta)
  {
    err = meta->setMdat(meta, (MP4AtomPtr)mdat);
    if(err) goto bail;
    err = meta->mdatMoved(meta, offset, mdat->dataSize + offset, -((s32)offset));
    if(err) goto bail;
  }
  ...
  if(movie->mdatList != NULL)
  {
    ...
    {
      err = MP4GetListEntry(movie->mdatList, i, (char **)&secondary_mdat);
      if(err) goto bail;
      ...
      err = mdat->addMdat(mdat, secondary_mdat);
      if(err) goto bail;
    }
...
MP4NewMovieFromHandle(MP4Movie *theMovie, MP4Handle movieH, u32 newMovieFlags)
{
  ...
  if(err) goto bail;
  err = MP4CreateMemoryInputStream(*movieH, handleSize, &moov->inputStream);
  if(err) goto bail;
  err = parseMovie(theMovie, moov, newMovieFlags);
  if(err) goto bail;
bail:
  TEST_RETURN(err);
  return err;
...
MP4_EXTERN(MP4Err) MP4AddAtomToMovie(MP4Movie theMovie, MP4GenericAtom the_atom)
{
  ...
  err = MP4NoErr;
  err = movieAtom->addAtom(movieAtom, (MP4AtomPtr)the_atom);
  if(err) goto bail;
               ...
               MP4DescriptorPtr *outDesc)
{
  ...
  if(err) goto bail;
  /*
  err = MP4CreateES_Descriptor( MP4ES_DescriptorTag, 0, 0, (MP4DescriptorPtr*) &esd ); if (err) goto
  bail; *esd = *esdInFile;
  */
  /* We don't want a shallow copy, we want a deep copy */
  err = MP4CopyDescriptor((MP4DescriptorPtr)esdInFile, (MP4DescriptorPtr *)&esd);
  if(err) goto bail;
  ...
  if(err) goto bail;
  if(dpnd && (dpnd->trackIDCount)) esd->dependsOnES = dpnd->trackIDs[0];

  /* JLF 11/00 */
  /* OCR dependancy */
  err = MP4GetTrackReferenceType(theTrack, MP4SyncTrackReferenceAtomType, &dpnd);
  if(err) goto bail;
  if(dpnd && (dpnd->trackIDCount)) esd->OCRESID = dpnd->trackIDs[0];
  ...
  */
  ...
  else
  {
    ...
    if(slconfig == NULL) BAILWITHERROR(MP4InvalidMediaErr);
...
MP4GetMovieIndTrack(MP4Movie theMovie, u32 trackIndex, MP4Track *outTrack)
{
  ...
  if(err) goto bail;
  if(aTrack == NULL) BAILWITHERROR(MP4BadDataErr)
...
{
  GETMOVIEATOM(theMovie);
  if(timeScale == 0) BAILWITHERROR(MP4BadParamErr);
  assert(movieAtom->setTimeScale);
  err = movieAtom->setTimeScale(movieAtom, timeScale);
  if(err) goto bail;
bail:
  return err;
}
...
ISOSetMovieBrand(ISOMovie theMovie, u32 brand, u32 minorversion)
{
  ...
  if(ftyp) err = ftyp->setBrand(ftyp, brand, minorversion);
  else
...
ISOSetMovieCompatibleBrand(ISOMovie theMovie, u32 brand)
{
  ...
  GETMOOV(theMovie);
  ftyp = (ISOFileTypeAtomPtr)moov->ftyp;
  if(ftyp) err = ftyp->addStandard(ftyp, brand);
  else
    BAILWITHERROR(MP4NoQTAtomErr)
bail:
  TEST_RETURN(err);
...
ISOGetMovieBrand(ISOMovie theMovie, u32 *brand, u32 *minorversion)
{
  ...
  ftyp = (ISOFileTypeAtomPtr)moov->ftyp;
  if(ftyp) err = ftyp->getBrand(ftyp, brand, minorversion);
  else
    BAILWITHERROR(MP4NoQTAtomErr)
bail:
  TEST_RETURN(err);
...
MP4NewMovieTrack(MP4Movie theMovie, u32 newTrackFlags, MP4Track *outTrack)
{
  ...
  GETMOVIEATOM(theMovie);
  err = movieAtom->newTrack(movieAtom, newTrackFlags, (MP4AtomPtr *)&trak);
  if(err) goto bail;
  err = trak->setMdat(trak, moov->mdat);
  if(err) goto bail;
  trak->moov = moov;
...
MP4NewMovieTrackWithID(MP4Movie theMovie, u32 newTrackFlags, u32 newTrackID, MP4Track *outTrack)
{
  MP4TrackAtomPtr trak;
  GETMOVIEATOM(theMovie);
  err = movieAtom->newTrackWithID(movieAtom, newTrackFlags, newTrackID, (MP4AtomPtr *)&trak);
  if(err) goto bail;
  err = trak->setMdat(trak, moov->mdat);
  if(err) goto bail;
  trak->moov = moov;
...
{
  return MP4NewMovieExt(outMovie, initialODID, OD_profileAndLevel, scene_profileAndLevel,
...
MP4Err MP4MovieCreateDescriptors(MP4Movie theMovie)
{
  ...
  if(movieRecord->fileType != ISOMPEG4FileType) return ISONoErr;

  err = movieAtom->calculateDuration(movieAtom);
  if(err) goto bail;

  iods = (MP4ObjectDescriptorAtomPtr)movieAtom->iods;
  if(iods == NULL) BAILWITHERROR(MP4NoErr); /* was MP4InvalidMediaErr -- iods is now optional */
  desc = (MP4DescriptorPtr)iods->descriptor;
  if(desc == NULL) BAILWITHERROR(MP4InvalidMediaErr);
  if((desc->tag != MP4_IOD_Tag) && (desc->tag != MP4_OD_Tag)) BAILWITHERROR(MP4InvalidMediaErr);
...
MP4Err MP4MovieAddTrackES_IDToIOD(MP4Movie theMovie, MP4Track theTrack)
{
  ...
  iods = (MP4ObjectDescriptorAtomPtr)movieAtom->iods;
  if(iods == NULL) BAILWITHERROR(MP4InvalidMediaErr);
  iod = (MP4InitialObjectDescriptorPtr)iods->descriptor;
  if((iod == NULL) || (iod->ES_ID_IncDescriptors == NULL)) BAILWITHERROR(MP4InvalidMediaErr);
...
MP4Err MP4GetCurrentTime(u64 *outTime)
{
  ...
  u64 ret;
  err = MP4NoErr;
  if(outTime == NULL) BAILWITHERROR(MP4BadParamErr)
#ifdef macintosh
...
ISONewMetaMovie(MP4Movie *outMovie, u32 handlertype, u32 brand, u32 minorversion)
{
  ...
  {
    MP4DataHandlerPtr dh;
    err = MP4CreateMdatDataHandler(mdat, &dh);
    if(err) goto bail;
    movie->inMemoryDataHandler = dh;
  }
  movie->fileType = ISOMPEG21FileType;
  err             = MJ2CreateFileTypeAtom(&ftyp);
  if(err) goto bail;
  err = ftyp->setBrand(ftyp, brand, minorversion);
  if(err) goto bail;
  err = ftyp->calculateSize((MP4AtomPtr)ftyp);
  if(err) goto bail;
  movie->ftyp = (MP4AtomPtr)ftyp;

  err = ISONewFileMeta((MP4Movie)movie, handlertype, &meta);
  if(err) goto bail;
...
MP4_EXTERN(MP4Err)
MP4NewMovieExt(MP4Movie *outMovie, u32 initialODID, u8 OD_profileAndLevel, u8 scene_profileAndLevel,
               u8 audio_profileAndLevel, u8 visual_profileAndLevel, u8 graphics_profileAndLevel,
               char *url, u8 IsExchangeFile)
{
  ...
  movie->referenceCount = 1;
  movie->prepend_handle = NULL;
  err                   = MP4GetCurrentTime(&now);
  if(err) goto bail;
  err = MP4CreateMovieAtom((MP4MovieAtomPtr *)&movie->moovAtomPtr);
  if(err) goto bail;
  moov = (MP4MovieAtomPtr)movie->moovAtomPtr;
  err  = MP4CreateMovieHeaderAtom(&mvhd);
  if(err) goto bail;
  mvhd->nextTrackID      = 1;
  mvhd->creationTime     = now;
  mvhd->modificationTime = now;
  err                    = moov->addAtom(moov, (MP4AtomPtr)mvhd);
  if(err) goto bail;
  moov->setTimeScale(moov, 600);

  if(initialODID != 0)
  {
    err = MP4CreateObjectDescriptorAtom(&iods);
    if(err) goto bail;

    /* here we have fun */
    if(IsExchangeFile)
    {
      err = MP4CreateObjectDescriptor(MP4_OD_Tag, 0, 0, (MP4DescriptorPtr *)&od);
      if(err) goto bail;
      ...
      err = iods->setDescriptor((MP4AtomPtr)iods, (char *)od);
      if(err) goto bail;
    }
    else
    {
      err = MP4CreateInitialObjectDescriptor(MP4_IOD_Tag, 0, 0, (MP4DescriptorPtr *)&iod);
      if(err) goto bail;
      iod->objectDescriptorID       = initialODID;
      iod->OD_profileAndLevel       = OD_profileAndLevel;
      iod->scene_profileAndLevel    = scene_profileAndLevel;
      ...
      err = iods->setDescriptor((MP4AtomPtr)iods, (char *)iod);
      if(err) goto bail;
    }
    err = moov->addAtom(moov, (MP4AtomPtr)iods);
    if(err) goto bail;
  }
  err         = MP4CreateMediaDataAtom(&mdat);
  movie->mdat = (MP4AtomPtr)mdat;
  ...
  err = MJ2CreateFileTypeAtom(&ftyp);
  if(err) goto bail;
  err = ftyp->setBrand(ftyp, ISOMpeg4V2Brand, 0);
  if(err) goto bail;
  err = ftyp->addStandard(ftyp, ISOISOBrand);
  if(err) goto bail;
  err = ftyp->calculateSize((MP4AtomPtr)ftyp);
  if(err) goto bail;
  movie->ftyp = (MP4AtomPtr)ftyp;
                                                ...
                                                MP4Handle outDescriptorH)
{
  ...
  if(iodAtom->ODSize)
  {
    ...
    */
    ...
    if(err) goto bail;
    err = odDesc->removeESDS((MP4DescriptorPtr)odDesc);
    if(err) goto bail;
