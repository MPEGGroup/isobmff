...
static MP4Err calculateSize(struct MP4DescriptorRecord *s)
{
  ...
  err = MP4GetListEntryCount(self->alternateToolIds, &numAlternates);
  if(err) goto bail;
  if(numAlternates != 0)
  {
  ...
  err = MP4GetListEntryCount(self->toolUrls, &numUrls);
  if(err) goto bail;
  self->size += 1; /* numUrls */
  for(i = 0; i < numUrls; i++)
  {
    ...
    err = MP4GetListEntry(self->toolUrls, i, (char **)&entry);
    if(err) goto bail;
...
static MP4Err serialize(struct MP4DescriptorRecord *s, char *buffer)
{
  ...
  err = MP4GetListEntryCount(self->alternateToolIds, &numAlternates);
  if(err) goto bail;
  flags = ((numAlternates > 0) ? 1 : 0) * 0x80 + ((self->toolParamDesc != NULL) ? 1 : 0) * 0x40;
  PUT8_V(flags);
  ...
  if(numAlternates > 0)
  {
    ...
    for(i = 0; i < numAlternates; i++)
    {
      ...
      err = MP4GetListEntry(self->alternateToolIds, i, (char **)&entry);
      if(err) goto bail;
  ...
  if(self->bytesWritten != self->size) BAILWITHERROR(MP4BadDataErr);
...
static MP4Err createFromInputStream(struct MP4DescriptorRecord *s, MP4InputStreamPtr inputStream)
{
  ...
  if(flags & 0x80) /* isAltGroup */
  {
    ...
    for(i = 0; i < numAlternates; i++)
    {
      ...
      err = MP4AddListEntry(id, self->alternateToolIds);
      if(err) goto bail;
    }
  }
  ...
  for(i = 0; i < numUrls; i++)
  {
    ...
    err = MP4AddListEntry(entry, self->toolUrls);
    if(err) goto bail;
  }
...
static void destroy(struct MP4DescriptorRecord *s)
{
  ...
  err                 = MP4NoErr;
  if(s == NULL) BAILWITHERROR(MP4BadParamErr);
  if(self->toolId) free(self->toolId);
  ...
  err = MP4DeleteLinkedList(self->alternateToolIds);
  if(err) goto bail;

  if(self->toolParamDesc)
  {
    if(self->toolParamDesc->data) free(self->toolParamDesc->data);
  ...
  err = MP4DeleteLinkedList(self->toolUrls);
  if(err) goto bail;
...
MP4Err MP4CreateIPMPTool(u32 tag, u32 size, u32 bytesRead, MP4DescriptorPtr *outDesc)
{
  ...
  err = MP4MakeLinkedList(&self->alternateToolIds);
  if(err) goto bail;
  err = MP4MakeLinkedList(&self->toolUrls);
  if(err) goto bail;
