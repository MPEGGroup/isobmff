...
static MP4Err getEntry(struct MP4DataReferenceAtom *self, u32 dataReferenceIndex,
                       struct MP4DataEntryAtom **outEntry)
{
  ...
  err = MP4GetListEntry(self->atomList, dataReferenceIndex - 1, (char **)&entry);
  if(err) goto bail;
  *outEntry = entry;
bail:
  TEST_RETURN(err);
...
static MP4Err getOffset(struct MP4DataReferenceAtom *self, u32 dataReferenceIndex, u64 *outOffset)
{
  ...
  err = MP4GetListEntry(self->atomList, dataReferenceIndex - 1, (char **)&entry);
  if(err) goto bail;
  err = entry->getOffset(entry, outOffset);
  if(err) goto bail;
bail:
  TEST_RETURN(err);
...
static MP4Err addSampleReference(struct MP4DataReferenceAtom *self, u32 sampleCount,
                                 u32 dataReferenceIndex, u64 dataOffset, MP4Handle sizesH)
{
  ...
  err = MP4GetListEntry(self->atomList, dataReferenceIndex - 1, (char **)&entry);
  if(err) goto bail;
  err = entry->addSampleReference(entry, dataOffset, sizesH);
  if(err) goto bail;
bail:
  TEST_RETURN(err);
...
static MP4Err addSamples(struct MP4DataReferenceAtom *self, u32 sampleCount, u32 dataReferenceIndex,
                         MP4Handle sampleH)
{
  MP4DataEntryAtomPtr entry;
  MP4Err err;
  ...
  err = MP4GetListEntry(self->atomList, dataReferenceIndex - 1, (char **)&entry);
  if(err) goto bail;
  err = entry->addSamples(entry, sampleH);
  if(err) goto bail;
bail:
  TEST_RETURN(err);
