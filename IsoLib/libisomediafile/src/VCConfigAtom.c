 ...
 * @file VCConfigAtom.c
 * @author julien Exp
 * @brief Implementation of the AVC configuration record 'avcC'
 * @version 0.1
 * @date 2002-09-20
 *
...
static void destroy(MP4AtomPtr s)
{
  ...
  if(self->spsList)
  {
    err = MP4DeleteLinkedList(self->spsList);
    if(err) goto bail;
    self->spsList = NULL;
  }
  if(self->ppsList)
  {
    err = MP4DeleteLinkedList(self->ppsList);
    if(err) goto bail;
    self->ppsList = NULL;
  }
  if(self->spsextList)
  {
    err = MP4DeleteLinkedList(self->spsextList);
    if(err) goto bail;
...
static MP4Err serialize(struct MP4Atom *s, char *buffer)
{
  ...
  {
    u32 count;
    err = MP4GetListEntryCount(self->spsList, &count);
    if(err) goto bail;
    ...
    for(i = 0; i < count; i++)
    {
      MP4Handle b;
      u32 the_size;
      err = MP4GetListEntry(self->spsList, i, (char **)&b);
      if(err) goto bail;
  ...
  {
    u32 count;
    err = MP4GetListEntryCount(self->ppsList, &count);
    if(err) goto bail;
    PUT8_V(count);
    ...
    for(i = 0; i < count; i++)
    {
      MP4Handle b;
      u32 the_size;
      err = MP4GetListEntry(self->ppsList, i, (char **)&b);
      if(err) goto bail;
      err = MP4GetHandleSize(b, &the_size);
      if(err) goto bail;
      PUT16_V(the_size);
  ...
  if(self->profile == 100 || self->profile == 110 || self->profile == 122 || self->profile == 144)
  {
    x = 0xFC | ((self->chroma_format) & 3);
    PUT8_V(x);
    x = 0xF8 | ((self->bit_depth_luma_minus8) & 7);
    PUT8_V(x);
    x = 0xF8 | ((self->bit_depth_chroma_minus8) & 7);
    PUT8_V(x);

    if(self->spsextList)
    {
      u32 count;
      err = MP4GetListEntryCount(self->spsextList, &count);
      if(err) goto bail;
      PUT8_V(count);
      ...
      for(i = 0; i < count; i++)
      {
        MP4Handle b;
        u32 the_size;
        err = MP4GetListEntry(self->spsextList, i, (char **)&b);
        if(err) goto bail;
        err = MP4GetHandleSize(b, &the_size);
        if(err) goto bail;
        PUT16_V(the_size);
...
static MP4Err createFromInputStream(MP4AtomPtr s, MP4AtomPtr proto, MP4InputStreamPtr inputStream)
{
  ...
  for(i = 0; i < count; i++)
  {
    ...
    err = MP4AddListEntry((void *)b, self->spsList);
    if(err) goto bail;
  }
  GET8_V(count);
  for(i = 0; i < count; i++)
  {
    ...
    err = MP4AddListEntry((void *)b, self->ppsList);
    if(err) goto bail;
  }
  ...
  {
    GET8_V(x);
    self->chroma_format = x & 3;
    GET8_V(x);
    self->bit_depth_luma_minus8 = x & 7;
    GET8_V(x);
    self->bit_depth_chroma_minus8 = x & 7;
    ...
    for(i = 0; i < count; i++)
    {
      ...
      err = MP4AddListEntry((void *)b, self->spsextList);
      if(err) goto bail;
    }
  }
...
static MP4Err getParameterSet(struct ISOVCConfigAtom *self, MP4Handle ps, u32 where, u32 index)
{
  ...
  {
  case AVCsps:
    err = MP4GetListEntry(self->spsList, index - 1, (char **)&b);
    if(err) goto bail;
    break;
  case AVCpps:
    err = MP4GetListEntry(self->ppsList, index - 1, (char **)&b);
    if(err) goto bail;
    break;
  case AVCspsext:
    err = MP4GetListEntry(self->spsextList, index - 1, (char **)&b);
    if(err) goto bail;
    break;
  default:
    BAILWITHERROR(MP4BadParamErr);
...
MP4Err MP4CreateVCConfigAtom(ISOVCConfigAtomPtr *outAtom)
{
  ...
  err                         = MP4MakeLinkedList(&self->spsList);
  if(err) goto bail;
  err = MP4MakeLinkedList(&self->ppsList);
  if(err) goto bail;
  err = MP4MakeLinkedList(&self->spsextList);
  if(err) goto bail;
