...
static MP4Err addSamplePads(struct MP4PaddingBitsAtom *self, u32 sampleCount, MP4Handle padsH)
{
  ...
  if(padsH != NULL)
  {
    ...
    err         = MP4GetHandleSize(padsH, &padsCount);
    if(err) goto bail;
    switch(padsCount)
    {
    case 0:
      break;
    case 1:
      defaultpad = *PaddingBits;
      break;
    default:
      dodefault = 0;
      assert(sampleCount == padsCount);
      break;
    }
  }
  ...
  TESTMALLOC(newpads);
  if(self->sampleCount > 0) memcpy(newpads, self->pads, self->sampleCount);
...
static MP4Err getPaddingBits(MP4AtomPtr s, u32 sampleNumber, u8 *outPad)
{
  ...
  err = MP4NoErr;
  if((self == NULL) || (outPad == NULL) || (sampleNumber > self->sampleCount) ||
     (sampleNumber == 0))
