...
static MP4Err addSizes( MP4AtomPtr s, u32 sampleCount, MP4Handle sizesH )
{
    ...
    if (self->default_sample_info_size == 0)
    {
        ...
        err = MP4GetHandleSize( sizesH, &sizeCount ); if (err) goto bail;
        ...
        {
            if ( sizeCount == 1 )
                self->sample_info_sizes[self->sample_count + i] = (u8) onlySize;
            else
...
MP4Err mergeSizes ( MP4AtomPtr s, MP4AtomPtr otherSaiz )
{
    ...
    self->sample_info_sizes = realloc(self->sample_info_sizes, self->sample_count + other->sample_count);
...
MP4Err MP4CreateSampleAuxiliaryInformationSizesAtom( MP4SampleAuxiliaryInformationSizesAtomPtr *outAtom )
{
	...
	self = (MP4SampleAuxiliaryInformationSizesAtomPtr) calloc( 1, sizeof(MP4SampleAuxiliaryInformationSizesAtom) );
	TESTMALLOC( self );
