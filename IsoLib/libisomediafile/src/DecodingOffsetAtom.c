...
static void destroy(MP4AtomPtr s)
{
  ...
  if(self->entryList)
  {
    ...
    {
      cttsEntryPtr pe;
      err = MP4GetListEntry(self->entryList, i, (char **)&pe);
      if(err) goto bail;
      free(pe);
    }
...
static MP4Err addSample(struct MP4CompositionOffsetAtom *self, u32 offset)
{
  ...
  if((current == NULL) || (current->decodingOffset != offset))
  {
    ...
    err                     = MP4AddListEntry(current, self->entryList);
    if(err) goto bail;
...
static MP4Err addSamples(struct MP4CompositionOffsetAtom *self, u32 sampleNumber, u32 sampleCount,
                         MP4Handle offsetsH)
{
  MP4Err err;
  u32 i;
  ...
  {
    s32 skipSamples;
    if(self->finalSampleNumber >= sampleNumber) BAILWITHERROR(MP4BadParamErr);
  ...
  if(offsetsH)
  {
    err = MP4GetHandleSize(offsetsH, &offsetCount);
    if(err) goto bail;
    offsetCount /= sizeof(u32);
    if(offsetCount == 1) onlyOffset = *(u32 *)*offsetsH;
