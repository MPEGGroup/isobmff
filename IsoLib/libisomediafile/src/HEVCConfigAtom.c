...
static void destroy( MP4AtomPtr s )
{
	...
	if ( self->numOfArrays )
	{
		for (i = 0; i < self->numOfArrays; i++) {
			err = MP4DeleteLinkedList(self->arrays[i].nalList); if (err) goto bail;
...
static MP4Err serialize( struct MP4Atom* s, char* buffer )
{
	...
	x = /*(1<<6) |*/ (1<<3) | (self->sps_temporal_id_nesting_flag << 2) | self->lengthSizeMinusOne+3;
	PUT8_V(x);
	...
	for (array_index = 0; array_index < self->numOfArrays; array_index++) {
		u32 count;
		err = MP4GetListEntryCount(self->arrays[array_index].nalList, &count); if (err) goto bail;
		...
		for (i=0; i<count; i++) {
			MP4Handle b;
			u32 the_size;
			err = MP4GetListEntry(self->arrays[array_index].nalList, i, (char **)&b); if (err) goto bail;
...
static MP4Err calculateSize( struct MP4Atom* s )
{
	...
	if (self->numOfArrays) {
		...
		for (i = 0; i < self->numOfArrays; i++) {
			u32 count;
			err = MP4GetListEntryCount(self->arrays[i].nalList, &count); if (err) goto bail;
			if (count >> 5) BAILWITHERROR(MP4BadParamErr);

			for (ii = 0; ii < count; ii++) {
				MP4Handle b;
				u32 the_size;
				err = MP4GetListEntry(self->arrays[i].nalList, ii, (char **)&b); if (err) goto bail;
				err = MP4GetHandleSize(b, &the_size);  if (err) goto bail;
...
static MP4Err createFromInputStream( MP4AtomPtr s, MP4AtomPtr proto, MP4InputStreamPtr inputStream )
{
	...
	for (array_index = 0; array_index < self->numOfArrays; array_index++) {
		...
		err = MP4MakeLinkedList(&self->arrays[array_index].nalList); if (err) goto bail;
		...
		for (i = 0; i < count; i++) {
			...
			err = MP4AddListEntry((void*)b, self->arrays[array_index].nalList); if (err) goto bail;
		}
	}
...
static MP4Err addParameterSet( struct ISOHEVCConfigAtom *self, MP4Handle ps, u32 nalu )
{
	...
	for (i = 0; i < 8; i++) {
		if (self->arrays[i].NALtype == nalu) {
			u32 nalCount = 0;
			err = MP4GetListEntryCount(self->arrays[i].nalList, &nalCount); if (err) goto bail;
			if (!nalCount) {
				self->numOfArrays++;
			}
			err = MP4AddListEntry((void*)b, self->arrays[i].nalList); if (err) goto bail;
			break;
		}
	}
...
static MP4Err getParameterSet( struct ISOHEVCConfigAtom *self, MP4Handle ps, u32 nalu, u32 index )
{
	...
	for (i = 0; i < self->numOfArrays; i++) {
		if (self->arrays[i].NALtype == nalu) {
			err = MP4GetListEntry(self->arrays[i].nalList, index - 1, (char**)&b); if (err) goto bail;
			break;
		}
	}
...
MP4Err MP4CreateHEVCConfigAtom( ISOHEVCConfigAtomPtr *outAtom )
{
	...
	for (i = 0; i < 8; i++) {
		err = MP4MakeLinkedList(&self->arrays[i].nalList); if (err) goto bail;
