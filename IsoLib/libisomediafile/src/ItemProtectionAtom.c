...
{
	MP4Err err;
	ISOItemProtectionAtomPtr self;
	u32 i;
	err = MP4NoErr;
	self = (ISOItemProtectionAtomPtr) s;
	if ( self == NULL )
...
static MP4Err addAtom( ISOItemProtectionAtomPtr self, MP4AtomPtr atom )
{
	MP4Err err;
...
MP4Err ISOCreateItemProtectionAtom( ISOItemProtectionAtomPtr *outAtom )
{
	...
	self = (ISOItemProtectionAtomPtr) calloc( 1, sizeof(ISOItemProtectionAtom) );
	TESTMALLOC( self );

	err = MP4CreateFullAtom( (MP4AtomPtr) self );
	if ( err ) goto bail;
	self->type = ISOItemProtectionAtomType;
	self->name                = "item protection";
