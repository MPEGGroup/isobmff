...
*/
/*
	$Id: AMRWPSpecificInfoAtom.c,v 1.1 2004/10/26 15:01:47 eanital Exp $
*/
...
{
	MP4Err err;
	MP4AMRWPSpecificInfoAtomPtr self;
	err = MP4NoErr;
	self = (MP4AMRWPSpecificInfoAtomPtr) s;
	if ( self == NULL )
...
{
	MP4Err err;
	MP4AMRWPSpecificInfoAtomPtr self = (MP4AMRWPSpecificInfoAtomPtr) s;
	err = MP4NoErr;
...
MP4Err MP4CreateAMRWPSpecificInfoAtom( MP4AMRWPSpecificInfoAtomPtr *outAtom )
{
	...
	self = (MP4AMRWPSpecificInfoAtomPtr) calloc( 1, sizeof(MP4AMRWPSpecificInfoAtom) );
	TESTMALLOC( self );

	err = MP4CreateBaseAtom( (MP4AtomPtr) self );
	if ( err ) goto bail;
	self->type =                MP4AMRWPSpecificInfoAtomType;
	self->name                  = "AMRWP specific info";
