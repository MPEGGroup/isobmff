...
static MP4Err calculateSize(struct MP4DescriptorRecord *s)
{
  ...
  err                           = MP4NoErr;
  if(s == 0) BAILWITHERROR(MP4BadParamErr);
...
#define SETFLAG(member, bit) self->member = flags & (1 << (bit)) ? 1 : 0
...
static MP4Err createFromInputStream(struct MP4DescriptorRecord *s, MP4InputStreamPtr inputStream)
{
  ...
  if(self->useTimestampsFlag == 0)
  {
    ...
    if(timestampLength & 0x7) timestampLength = 1 + timestampLength / 8;
    else
      timestampLength /= 8;
  ...
  if(self->bytesRead == (self->size - 1))
  {
    u32 junk;
    GET8_V_MSG(junk, NULL);
  }
...
MP4NewSLConfig(MP4SLConfigSettingsPtr settings, MP4SLConfig *outSLConfig)
{
  ...
  err = MP4CreateSLConfigDescriptor(MP4SLConfigDescriptorTag, 0, 0, &desc);
  if(err) goto bail;
  err = MP4SetSLConfigSettings((MP4SLConfig)desc, settings);
  if(err) goto bail;
