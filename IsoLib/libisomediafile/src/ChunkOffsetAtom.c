...
static u32 getChunkCount(MP4AtomPtr self) { return ((MP4ChunkOffsetAtomPtr)self)->entryCount; }
...
static MP4Err addOffset(struct MP4ChunkOffsetAtom *s, u64 offset)
{
  ...
  if((self->entryCount * sizeof(u64)) > self->allocatedSize)
  {
    ...
    if(self->offsets != NULL) self->offsets = (u64 *)realloc(self->offsets, self->allocatedSize);
    else
...
static MP4Err calculateSize(struct MP4Atom *s)
{
  ...
  if(err) goto bail;
  if(self->type == MP4ChunkOffsetAtomType) self->size += 4 + (4 * self->entryCount);
  else
...
static MP4Err mdatMoved(MP4ChunkOffsetAtomPtr self, u32 firstchunk, u32 lastchunk, u64 mdatBase,
                        u64 mdatEnd, s32 mdatOffset)
{
  MP4Err err;
  u32 j;

  err = MP4NoErr;
  if((self == NULL) || (firstchunk == 0) || (firstchunk > self->entryCount) || (lastchunk == 0) ||
     (lastchunk > self->entryCount))
    BAILWITHERROR(MP4BadParamErr)
