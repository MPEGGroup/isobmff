 ...
 * @param first_sps The sequence parameter set (MUST be passed) used to get the profile, level, etc.
 * It will only be added to the configuration if a picture parameter set is also present. 
...
/**
 * @brief This adds another parameter set (which is not, in fact, inspected), to the configuration.
 * @ingroup SampleDescr
 * 
...
/**
 * @brief Creates a new meta box (atom), with the indicated ‘handler’ type, in the movie box (atom).
 * 
 * If there is already a meta box present, a additional meta box atom will be created and the new 
 * meta box will be added inside.
 * 
...
/**
 * @brief Adds a meta box relation box to the additional meta data box, related to first and second 
 * meta box.
 * 
 ...
 * 
 * @param is_full_atom This flag can be used to insert the four bytes of version and flags required 
 * in a full atom.
 */
 ...
 * @brief Finds the box of the given type inside the meta box, and returns its contents. 
 * 
 * Like ISOGetPrimaryData(), but probably more useful, as if a box of the given type does not exist,
 * finds the primary item and returns its data. Gives an error indication if neither the box nor the
 * primary item exist.
 */
 ...
 * 
 * The input parameter meta-type, if non-zero, can be used to check that the associated meta-data is
 * of the given type. The output always supplies the type.
 */
 ...
 * concatenating them.
 * 
 * The returned base offset will be adjusted to represent the offset in the resulting handle (which 
 * will generally match the value passed in to ISOAddMetaItem(), typically zero).
 */
...
/**
 * @brief Adds an arbitrary atom to the scheme information of the associated protection information.
 */
...
ISO_EXTERN(MP4Err) ISOAddMetaItemProperty(ISOMetaItem item, MP4GenericAtom *itemProperty, 
                                          u8 essential);
/**
 * @brief Retruns an array of MP4GenericAtom pointers, which represent the item properties found for
 * the item given.
 * 
