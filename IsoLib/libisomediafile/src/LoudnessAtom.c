...
static MP4Err addAtom( MP4LoudnessAtomPtr self, MP4AtomPtr atom )
{
    ...
    if (atom->type == MP4TrackLoudnessInfoAtomType)
        err = MP4AddListEntry( atom, self->trackLoudnessInfoList ); if (err) goto bail;
    if (atom->type == MP4AlbumLoudnessInfoAtomType)
        err = MP4AddListEntry( atom, self->albumLoudnessInfoList ); if (err) goto bail;
bail:
	TEST_RETURN( err );
...
MP4Err MP4CreateLoudnessAtom( MP4LoudnessAtomPtr *outAtom )
{
    ...
    err = MP4MakeLinkedList( &self->trackLoudnessInfoList ); if (err) goto bail;
    err = MP4MakeLinkedList( &self->albumLoudnessInfoList ); if (err) goto bail;
