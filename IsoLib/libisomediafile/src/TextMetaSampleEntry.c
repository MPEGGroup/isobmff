...
static MP4Err readstring(char **b, MP4TextMetaSampleEntryAtomPtr self,
                         MP4InputStreamPtr inputStream, char *fieldname)
{
  u32 byte;
  MP4Err err;
  char temp[1024];
...
static MP4Err createFromInputStream(MP4AtomPtr s, MP4AtomPtr proto, MP4InputStreamPtr inputStream)
{
  ...
  if(self->size > self->bytesRead)
  {
    err = readstring(&(self->mime_format), self, inputStream, "mime_format");
    if(err) goto bail;
  }
  else
    self->mime_format = NULL;
