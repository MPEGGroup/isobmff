...
static MP4Err addSamples( struct MP4SampleDependencyAtom *self, u32 count )
{
	...
	err = MP4NoErr;
	err = ensureSize( self, (self->sampleCount + count)*sizeof(u8) ); if (err) goto bail;
									...
									MP4Handle dependencies )
{
	...
	for (i=0; i<count; i++) *p++ = *dependency++;
									...
									u8* dependency  )
{
	...
	if (sampleNumber > self->sampleCount) 
		 *dependency = 0;
	else *dependency = (self->dependency)[sampleNumber - 1];
...
static MP4Err createFromInputStream( MP4AtomPtr s, MP4AtomPtr proto, MP4InputStreamPtr inputStream )
{
	...
	err = ensureSize( self, self->sampleCount ); if (err) goto bail;
