...
static MP4Err getOffset(struct MP4DataInformationAtom *self, u32 dataReferenceIndex, u64 *outOffset)
{
  ...
  err = MP4NoErr;
  if(self->dataReference == NULL) BAILWITHERROR(MP4InvalidMediaErr);
  if(dataReferenceIndex == 0) BAILWITHERROR(MP4BadParamErr);
...
static MP4Err addSampleReference(struct MP4DataInformationAtom *self, u32 sampleCount,
                                 u32 dataReferenceIndex, u64 dataOffset, MP4Handle sizesH)
{
  ...
  if(dataReferenceIndex == 0) BAILWITHERROR(MP4BadParamErr);
  dref = (MP4DataReferenceAtomPtr)self->dataReference;
  err  = dref->addSampleReference(dref, sampleCount, dataReferenceIndex, dataOffset, sizesH);
  if(err) goto bail;
bail:
  TEST_RETURN(err);
...
static MP4Err addSamples(struct MP4DataInformationAtom *self, u32 sampleCount,
                         u32 dataReferenceIndex, MP4Handle sampleH)
{
  MP4DataReferenceAtomPtr dref;
  MP4Err err;
  ...
  if(dataReferenceIndex == 0) BAILWITHERROR(MP4BadParamErr);
  dref = (MP4DataReferenceAtomPtr)self->dataReference;
  err  = dref->addSamples(dref, sampleCount, dataReferenceIndex, sampleH);
  if(err) goto bail;
bail:
  TEST_RETURN(err);
...
static MP4Err addAtom(MP4DataInformationAtomPtr self, MP4AtomPtr atom)
{
  ...
  case MP4DataReferenceAtomType:
    if(self->dataReference) BAILWITHERROR(MP4BadDataErr)
