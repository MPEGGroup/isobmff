...
static void destroy( MP4AtomPtr s )
{
	MP4Err err;
	u32 i;
	MP4TrackGroupAtomPtr self;
	err = MP4NoErr;
	self = (MP4TrackGroupAtomPtr) s;
	if ( self == NULL )
...
{
	MP4Err err;
	MP4TrackGroupAtomPtr self = (MP4TrackGroupAtomPtr) s;
	err = MP4NoErr;
...
static MP4Err findAtomOfType( struct MP4TrackGroupAtom *self, u32 atomType, MP4AtomPtr *outAtom )
{
	MP4Err err;
	u32 entryCount;
	u32 i;
...
static MP4Err addAtom( MP4TrackGroupAtomPtr self, MP4AtomPtr atom )
{
	MP4Err err;
	if ( self == 0 )
		BAILWITHERROR( MP4BadParamErr );
...
{
	/* PARSE_ATOM_LIST(MP4TrackGroupAtom) */
	...
	while ( self->bytesRead < self->size )
	{
		...
		err = MP4ParseAtomUsingProtoList( (MP4InputStreamPtr) inputStream, 
                                      NoProtos, MP4_FOUR_CHAR_CODE('m', 's', 'r', 'c'),
                                      &atom);
			if (err) goto bail;
...
MP4Err MP4CreateTrackGroupAtom( MP4TrackGroupAtomPtr *outAtom )
{
	MP4Err err;
	MP4TrackGroupAtomPtr self;	
	self = (MP4TrackGroupAtomPtr) calloc( 1, sizeof(MP4TrackGroupAtom) );
	TESTMALLOC( self )
