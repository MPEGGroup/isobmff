...
static MP4Err addDescriptor(struct MP4DescriptorRecord *s, MP4DescriptorPtr desc)
{
  ...
  {
    err = MP4AddListEntry(desc, self->ipmpToolDescriptors);
    if(err) goto bail;
  }
  else
...
MP4Err MP4CreateIPMPToolDescriptorUpdate(u32 tag, u32 size, u32 bytesRead,
                                         MP4DescriptorPtr *outDesc)
{
  SETUP_BASE_DESCRIPTOR(MP4IPMPToolDescriptorUpdate)
  self->addDescriptor = addDescriptor;
  err                 = MP4MakeLinkedList(&self->ipmpToolDescriptors);
  if(err) goto bail;
