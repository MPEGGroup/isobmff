...
MJ2NewMovie(ISOMovie *outMovie)
{
  ...
  if(err) goto bail;
  err = ftyp->setBrand(ftyp, MJ2JPEG2000Brand, 0);
  if(err) goto bail;
  ...
  err = MJ2CreateSignatureAtom(&sgnt);
  if(err) goto bail;
  err = sgnt->calculateSize((MP4AtomPtr)sgnt);
  if(err) goto bail;
...
#ifdef JP2KSTILLIMAGE
  err = MJ2CreateHeaderAtom(&jp2h);
  if(err) goto bail;
  err = jp2h->calculateSize((MP4AtomPtr)jp2h);
  if(err) goto bail;
...

  {
    ...
    err = MJ2CreateImageHeaderAtom(&imag);
    if(err) goto bail;
    err = MJ2CreateBitsPerComponentAtom(&bpco);
    if(err) goto bail;
    err = MJ2CreateColorSpecificationAtom(&colr);
    if(err) goto bail;

    err = imag->calculateSize((MP4AtomPtr)imag);
    if(err) goto bail;
    err = bpco->calculateSize((MP4AtomPtr)bpco);
    if(err) goto bail;
    err = colr->calculateSize((MP4AtomPtr)colr);
    if(err) goto bail;
