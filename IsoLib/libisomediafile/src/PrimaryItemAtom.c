...
{
	MP4Err err;
	ISOPrimaryItemAtomPtr self = (ISOPrimaryItemAtomPtr) s;
	err = MP4NoErr;
...
{
	MP4Err err;
	ISOPrimaryItemAtomPtr self;
	
	self = (ISOPrimaryItemAtomPtr) calloc( 1, sizeof(ISOPrimaryItemAtom) );
	TESTMALLOC( self )

	err = MP4CreateFullAtom( (MP4AtomPtr) self );
	if ( err ) goto bail;
	self->type = ISOPrimaryItemAtomType;
	self->name					= "primary item";
	self->flags					= 0;
