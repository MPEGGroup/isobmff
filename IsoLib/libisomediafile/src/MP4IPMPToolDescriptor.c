...
static MP4Err addIPMPXData(struct MP4DescriptorRecord *s, MP4IPMPXDataPtr data)
{
  ...
  err                           = MP4NoErr;
  err                           = MP4AddListEntry(data, self->ipmpxData);
  if(err) goto bail;
bail:
  TEST_RETURN(err);
...
static MP4Err calculateSize(struct MP4DescriptorRecord *s)
{
  ...
  if(self->ipmpToolId->upperPart == 0 && self->ipmpToolId->lowerPart == 0)
  {
  ...
  else
  {
    ...
    err = MP4GetListEntryCount(self->ipmpxData, &ipmpxDataCount);
    if(err) goto bail;
    for(i = 0; i < ipmpxDataCount; i++)
    {
      MP4IPMPXDataPtr entry;
      err = MP4GetListEntry(self->ipmpxData, i, (char **)&entry);
      if(err) goto bail;
...
static MP4Err serialize(struct MP4DescriptorRecord *s, char *buffer)
{
  ...
  if(self->ipmpToolId->upperPart == 0 && self->ipmpToolId->lowerPart == 0)
  {
    if(self->url == NULL) BAILWITHERROR(MP4BadDataErr);
  ...
  else
  {
    ...
    if(self->initData != NULL)
    {
      err = self->initData->serialize(self->initData, buffer);
      if(err) goto bail;
...
static MP4Err createFromInputStream(struct MP4DescriptorRecord *s, MP4InputStreamPtr inputStream)
{
  ...
  else
  {
    ...
    if(initFlag & 0x80)
    {
      err = MP4CreateIPMPInitialize(&self->initData);
      if(err) goto bail;
      err = self->initData->createFromInputStream(self->initData, inputStream);
      if(err) goto bail;
    ...
    for(i = 0; i < ipmpxDataCount; i++)
    {
      ...
      err = MP4ParseIPMPXData(inputStream, &data);
      if(err) goto bail;
      self->bytesRead += data->size;
      err = addIPMPXData(s, data);
      if(err) goto bail;
    }
  }
...
static void destroy(struct MP4DescriptorRecord *s)
{
  ...
  err                           = MP4NoErr;
  if(s == NULL) BAILWITHERROR(MP4BadParamErr);
  if(self->ipmpToolId) free(self->ipmpToolId);
  if(self->url)
  {
    if(self->url->data) free(self->url->data);
    free(self->url);
  }
  if(self->initData)
  {
...
MP4Err MP4CreateIPMPToolDescriptor(u32 tag, u32 size, u32 bytesRead, MP4DescriptorPtr *outDesc)
{
  ...
  err = MP4MakeLinkedList(&self->ipmpxData);
  if(err) goto bail;
