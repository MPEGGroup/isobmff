...
MP4Err MP4CreateDataHandler(MP4InputStreamPtr inputStream, MP4DataEntryAtomPtr dataEntry,
...
{
  if((dataEntry->flags == 1) && (dataEntry->mdat != NULL)) return dataHandler->close(dataHandler);
  else
...
{
  if(dataEntry->flags == 1) return MP4NoErr;
