...
static ISOErr getBrand(struct MP4SegmentTypeAtom *self, u32 *standard, u32 *minorversion)
{
...
static u32 getStandard(struct MP4SegmentTypeAtom *self, u32 standard)
{
  u32 i;
  u32 outval;
...
static ISOErr setBrand(struct MP4SegmentTypeAtom *self, u32 standard, u32 minorversion)
{
  u32 oldstandard;
  MP4Err err;
...
ISOErr MP4CreateSegmentTypeAtom(MP4SegmentTypeAtomPtr *outAtom)
{
  ...
  self = (MP4SegmentTypeAtomPtr)calloc(1, sizeof(MP4SegmentTypeAtom));
  TESTMALLOC(self);

  err = MP4CreateFullAtom((MP4AtomPtr)self); /* Full atom */
  if(err) goto bail;

  self->type                  = MP4SegmentTypeAtomType;
  self->name                  = "SegmentTypeBox";
