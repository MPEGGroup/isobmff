...
static MP4Err setSampleDependency( struct MP4MediaAtom *self, s32 sample_index, MP4Handle dependencies  )
{
   MP4Err err;
   MP4MediaInformationAtomPtr minf;
   ...
   err = minf->setSampleDependency( minf, sample_index, dependencies ); if (err) goto bail;
  bail:
   TEST_RETURN( err );
...
static MP4Err getSampleDependency( struct MP4MediaAtom *self, u32 sampleNumber, u8* dependency  )
{
   ...
   assert( minf );
    ...
    err = minf->getSampleDependency( minf, sampleNumber, dependency ); if (err) goto bail;
  bail:
   TEST_RETURN( err );
