...
MP4Err MP4CreateDRCAtom                ( u32 atomType, MP4AtomPtr *outAtom )
{
    ...
    logMsg(LOGLEVEL_TRACE, "MP4CreateDRCAtom called with type: \"%s\"", typeString);
    
	switch ( atomType )
	{
            
        case AudioSigned16BitLittleEndianSampleEntryType:
            err = MP4CreateDrcAudioSampleEntryAtom      ( (MP4AudioSampleEntryAtomPtr*) &newAtom );
            break;
            
        case DRCCoefficientBasicAtomType:
			err = MP4CreateDRCCoefficientBasicAtom      ( (DRCCoefficientBasicAtomPtr*) &newAtom );
			break;
			
		case DRCInstructionsBasicAtomType:
			err = MP4CreateDRCInstructionsBasicAtom     ( (DRCInstructionsBasicAtomPtr*) &newAtom );
			break;
			
		case DRCCoefficientUniDRCAtomType:
			err = MP4CreateDRCCoefficientUniDRCAtom     ( (DRCCoefficientUniDRCAtomPtr*) &newAtom );
			break;
			
		case DRCInstructionsUniDRCAtomType:
			err = MP4CreateDRCInstructionsUniDRCAtom    ( (DRCInstructionsUniDRCAtomPtr*) &newAtom );
			break;
...
MP4Err MP4ParseDRCAtomUsingProtoList   ( MP4InputStreamPtr inputStream, u32* protoList, u32 defaultAtom, MP4AtomPtr *outAtom )
{
	...
	err = MP4CreateDRCAtom( useDefaultAtom ? defaultAtom : atomProto->type, &newAtom ); if ( err ) goto bail;
...
MP4Err MP4ParseDRCAtom                 ( MP4InputStreamPtr inputStream, MP4AtomPtr *outAtom )
{
    ...
    return  MP4ParseDRCAtomUsingProtoList( inputStream, NULL, 0, outAtom  );
