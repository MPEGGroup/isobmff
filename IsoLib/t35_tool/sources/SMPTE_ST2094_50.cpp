...
void printDebug(const std::string &varName, uint16_t varValue, uint8_t nbBits, int verboseLevel)
{
  ...
  std::cout << varValue << " | ";
  switch(nbBits)
  { // bitset need constant
  case 1:
    std::cout.width(16);
    std::cout << std::bitset<1>(varValue).to_string() << "\n";
    break;
  case 2:
    std::cout.width(16);
    std::cout << std::bitset<2>(varValue).to_string() << "\n";
    break;
  case 3:
    std::cout.width(16);
    std::cout << std::bitset<3>(varValue).to_string() << "\n";
    break;
  case 4:
    std::cout.width(16);
    std::cout << std::bitset<4>(varValue).to_string() << "\n";
    break;
  case 5:
    std::cout.width(16);
    std::cout << std::bitset<5>(varValue).to_string() << "\n";
    break;
  case 6:
    std::cout.width(16);
    std::cout << std::bitset<6>(varValue).to_string() << "\n";
    break;
  case 7:
    std::cout.width(16);
    std::cout << std::bitset<7>(varValue).to_string() << "\n";
    break;
  case 8:
    std::cout.width(16);
    std::cout << std::bitset<8>(varValue).to_string() << "\n";
    break;
  case 16:
    std::cout.width(16);
    std::cout << std::bitset<16>(varValue).to_string() << "\n";
    break;
  default:
    break;
  }
...
void printBinaryData(std::vector<uint8_t> binary_data, int verboseLevel)
{
  ...
  std::cout << "Binary data decoded, paste in external tool -> "
               "https://ccameron-chromium.github.io/agtm-demo/parse.html"
            << std::endl;
                  ...
                  int verboseLevel)
{
  ...
  payloadBinaryData->payload[payloadBinaryData->byteIdx] =
    payloadBinaryData->payload[payloadBinaryData->byteIdx] +
    (decValue << (7 - payloadBinaryData->bitIdx));
...
void push_bits(struct BinaryData *payloadBinaryData, uint8_t value, uint8_t nbBits,
               const std::string &varName, int verboseLevel)
{
  payloadBinaryData->payload[payloadBinaryData->byteIdx] =
    payloadBinaryData->payload[payloadBinaryData->byteIdx] +
    (value << (8 - nbBits - payloadBinaryData->bitIdx));
  ...
  {
    logMsg(LOGLEVEL_ERROR, "push_bits exceeded a byte for %s while trying to add %d bits",
           varName.c_str(), nbBits);
  }
}
...
{
  uint8_t decValue =
    (payloadBinaryData->payload[payloadBinaryData->byteIdx] >> (7 - payloadBinaryData->bitIdx)) &
    0x01;
...
{
  uint8_t decValue =
    uint8_t(payloadBinaryData->payload[payloadBinaryData->byteIdx] << payloadBinaryData->bitIdx) >>
    ((8 - nbBits));
  payloadBinaryData->bitIdx += nbBits;
  if(payloadBinaryData->bitIdx == 8)
  {
  ...
  {
    logMsg(LOGLEVEL_ERROR, "pull_bits exceeded a byte for %s while trying to add %d bits",
           varName.c_str(), nbBits);
  }
                     ...
                     int verboseLevel)
{
  ...
  {
    decValue = uint16_t(payloadBinaryData->payload[payloadBinaryData->byteIdx]) << 8;
    payloadBinaryData->byteIdx++;
    decValue = decValue + uint16_t(payloadBinaryData->payload[payloadBinaryData->byteIdx]);
    payloadBinaryData->byteIdx++;
...
// Setters
void SMPTE_ST2094_50::setTimeIntervalStart(uint32_t frame_start)
{
  timeI.timeIntervalStart = frame_start;
}
void SMPTE_ST2094_50::setTimeintervalDuration(uint32_t frame_duration)
{
  timeI.timeintervalDuration = frame_duration;
}
void SMPTE_ST2094_50::setVerboseLevel(int level)
{
  ...
  {
    logMsg(LOGLEVEL_WARNING, "Invalid verbose level %d, keeping current level %d", level,
           verboseLevel);
  }
}
...
bool SMPTE_ST2094_50::decodeJsonToMetadataItems(nlohmann::json j)
{
  ...
  {
    logMsg(LOGLEVEL_ERROR, "SMPTEST2094_50 not found in json file");
  }
  ...
  if(!cvtDict.contains("HeadroomAdaptiveToneMapping") ||
     !cvtDict["HeadroomAdaptiveToneMapping"].is_object())
  {
  ...
  {
    std::vector<float> gainAppSpaceChrom =
      hatmDict["gainApplicationSpaceChromaticities"].get<std::vector<float>>();
    if(gainAppSpaceChrom.size() != MAX_NB_CHROMATICITIES)
    {
      logMsg(LOGLEVEL_ERROR, "gainApplicationSpaceChromaticities array size (%zu) != %d",
             gainAppSpaceChrom.size(), MAX_NB_CHROMATICITIES);
      return true;
    }
  ...
  if(cvt.hatm.numAlternateImages >= 1)
  {
    ...
    if(hatmDict.contains("alternateHdrHeadroom"))
    {
      ...
      if(!alternateHdrHeadroomArray.empty())
      {
        ...
        {
          logMsg(LOGLEVEL_ERROR,
                 "alternateHdrHeadroom array count (%zu) does not match numAlternateImages (%u)",
    ...
    {
      logMsg(LOGLEVEL_ERROR,
             "ColorGainFunction array count (%zu) does not match numAlternateImages (%u)",
    ...
    for(uint32_t i = 0; i < cvt.hatm.numAlternateImages; i++)
    {
      ...
      auto parseControlPointArray = [&](const std::string &key, std::vector<float> &target) -> bool
      {
        ...
        if(value.is_array())
        {
          ...
          {
            logMsg(LOGLEVEL_ERROR, "%s array count mismatch for ColorGainFunction[%u]", key.c_str(),
                   i);
            return false;
          }
          target = arr;
      ...
      {
        logMsg(LOGLEVEL_ERROR, "Failed to parse gainCurveControlPointX for ColorGainFunction[%u]",
               i);
        return false;
      }
      ...
      {
        logMsg(LOGLEVEL_ERROR, "Failed to parse gainCurveControlPointY for ColorGainFunction[%u]",
               i);
        return true;
      }
      ...
      if(gainCurveDict.contains("gainCurveControlPointM"))
      {
        ...
        {
          logMsg(LOGLEVEL_WARNING,
                 "Failed to parse gainCurveControlPointM for ColorGainFunction[%u], continuing "
                 "without it",
                 i);
...
void SMPTE_ST2094_50::convertMetadataItemsToSyntaxElements()
{
  ...
  if(isHeadroomAdaptiveToneMap)
  {
    ...
    if(!isReferenceWhiteToneMapping)
    {
      ...
      {
        logMsg(LOGLEVEL_ERROR,
               "cgf array size (%zu) is less than numAlternateImages (%u). JSON data is incomplete "
               "or malformed.",
               cvt.hatm.cgf.size(), cvt.hatm.numAlternateImages);
        return;
      }

      // Check if the primary combination is known
      if(std::abs(cvt.hatm.gainApplicationSpaceChromaticities[0] - 0.64) <
           P_GAIN_APPLICATION_SPACE_CHROMATICITY &&
         std::abs(cvt.hatm.gainApplicationSpaceChromaticities[1] - 0.33) <
           P_GAIN_APPLICATION_SPACE_CHROMATICITY &&
         std::abs(cvt.hatm.gainApplicationSpaceChromaticities[2] - 0.30) <
           P_GAIN_APPLICATION_SPACE_CHROMATICITY &&
         std::abs(cvt.hatm.gainApplicationSpaceChromaticities[3] - 0.60) <
           P_GAIN_APPLICATION_SPACE_CHROMATICITY &&
         std::abs(cvt.hatm.gainApplicationSpaceChromaticities[4] - 0.15) <
           P_GAIN_APPLICATION_SPACE_CHROMATICITY &&
         std::abs(cvt.hatm.gainApplicationSpaceChromaticities[5] - 0.06) <
           P_GAIN_APPLICATION_SPACE_CHROMATICITY &&
         std::abs(cvt.hatm.gainApplicationSpaceChromaticities[6] - 0.3127) <
           P_GAIN_APPLICATION_SPACE_CHROMATICITY &&
         std::abs(cvt.hatm.gainApplicationSpaceChromaticities[7] - 0.3290) <
           P_GAIN_APPLICATION_SPACE_CHROMATICITY)
      {
        elm.gain_application_space_chromaticities_mode = 0;
      }
      else if(std::abs(cvt.hatm.gainApplicationSpaceChromaticities[0] - 0.68) <
                P_GAIN_APPLICATION_SPACE_CHROMATICITY &&
              std::abs(cvt.hatm.gainApplicationSpaceChromaticities[1] - 0.32) <
                P_GAIN_APPLICATION_SPACE_CHROMATICITY &&
              std::abs(cvt.hatm.gainApplicationSpaceChromaticities[2] - 0.265) <
                P_GAIN_APPLICATION_SPACE_CHROMATICITY &&
              std::abs(cvt.hatm.gainApplicationSpaceChromaticities[3] - 0.69) <
                P_GAIN_APPLICATION_SPACE_CHROMATICITY &&
              std::abs(cvt.hatm.gainApplicationSpaceChromaticities[4] - 0.15) <
                P_GAIN_APPLICATION_SPACE_CHROMATICITY &&
              std::abs(cvt.hatm.gainApplicationSpaceChromaticities[5] - 0.06) <
      ...
      else if(std::abs(cvt.hatm.gainApplicationSpaceChromaticities[0] - 0.708) <
                P_GAIN_APPLICATION_SPACE_CHROMATICITY &&
              std::abs(cvt.hatm.gainApplicationSpaceChromaticities[1] - 0.292) <
                P_GAIN_APPLICATION_SPACE_CHROMATICITY &&
              std::abs(cvt.hatm.gainApplicationSpaceChromaticities[2] - 0.17) <
                P_GAIN_APPLICATION_SPACE_CHROMATICITY &&
              std::abs(cvt.hatm.gainApplicationSpaceChromaticities[3] - 0.797) <
                P_GAIN_APPLICATION_SPACE_CHROMATICITY &&
              std::abs(cvt.hatm.gainApplicationSpaceChromaticities[4] - 0.131) <
                P_GAIN_APPLICATION_SPACE_CHROMATICITY &&
              std::abs(cvt.hatm.gainApplicationSpaceChromaticities[5] - 0.046) <
                P_GAIN_APPLICATION_SPACE_CHROMATICITY &&
              std::abs(cvt.hatm.gainApplicationSpaceChromaticities[6] - 0.3127) <
                P_GAIN_APPLICATION_SPACE_CHROMATICITY &&
              std::abs(cvt.hatm.gainApplicationSpaceChromaticities[7] - 0.3290) <
      ...
      else
      {
        ...
        {
          elm.gain_application_space_chromaticities[iCh] =
            uint16_t(cvt.hatm.gainApplicationSpaceChromaticities[iCh] *
                       Q_GAIN_APPLICATION_SPACE_CHROMATICITY +
                     0.5f);
        }
      }
      ...
      elm.has_common_component_mix_params_flag =
        true; // Check if all component mixing uses the same parameters
      elm.has_common_curve_params_flag = true; // Check if all alternate have the same number of
                                               // control points, x position and interpolation
      for(uint16_t iAlt = 0; iAlt < cvt.hatm.numAlternateImages; iAlt++)
      {
        if(fabs(cvt.hatm.alternateHdrHeadroom[iAlt] - cvt.hatm.baselineHdrHeadroom) <
           P_HDR_HEADROOM)
        {
          logMsg(LOGLEVEL_ERROR, "alternateHdrHeadroom[%d] cannot be equal to baselineHdrHeadroom",
                 iAlt);
          return;
        }
        elm.alternate_hdr_headrooms[iAlt] =
          uint16_t(cvt.hatm.alternateHdrHeadroom[iAlt] * Q_HDR_HEADROOM + 0.5f);
        ...
        // Component mixing
        if(std::abs(cvt.hatm.cgf[iAlt].cm.componentMixRed) < P_COMPONENT_MIXING_COEFFICIENT &&
        ...
        else if(std::abs(cvt.hatm.cgf[iAlt].cm.componentMixRed) < P_COMPONENT_MIXING_COEFFICIENT &&
                std::abs(cvt.hatm.cgf[iAlt].cm.componentMixGreen) <
                  P_COMPONENT_MIXING_COEFFICIENT &&
                std::abs(cvt.hatm.cgf[iAlt].cm.componentMixBlue) < P_COMPONENT_MIXING_COEFFICIENT &&
                std::abs(cvt.hatm.cgf[iAlt].cm.componentMixMax) < P_COMPONENT_MIXING_COEFFICIENT &&
                std::abs(cvt.hatm.cgf[iAlt].cm.componentMixMin) < P_COMPONENT_MIXING_COEFFICIENT &&
                std::abs(cvt.hatm.cgf[iAlt].cm.componentMixComponent - 1.0) <
                  P_COMPONENT_MIXING_COEFFICIENT)
        {
          elm.component_mixing_type[iAlt] = 1;
        }
        else if(std::abs(cvt.hatm.cgf[iAlt].cm.componentMixRed - (1.0 / 6.0)) <
                  P_COMPONENT_MIXING_COEFFICIENT &&
                std::abs(cvt.hatm.cgf[iAlt].cm.componentMixGreen - (1.0 / 6.0)) <
                  P_COMPONENT_MIXING_COEFFICIENT &&
                std::abs(cvt.hatm.cgf[iAlt].cm.componentMixBlue - (1.0 / 6.0)) <
                  P_COMPONENT_MIXING_COEFFICIENT &&
                std::abs(cvt.hatm.cgf[iAlt].cm.componentMixMax - (1.0 / 2.0)) <
                  P_COMPONENT_MIXING_COEFFICIENT &&
                std::abs(cvt.hatm.cgf[iAlt].cm.componentMixMin) < P_COMPONENT_MIXING_COEFFICIENT &&
                std::abs(cvt.hatm.cgf[iAlt].cm.componentMixComponent) <
                  P_COMPONENT_MIXING_COEFFICIENT)
        {
          elm.component_mixing_type[iAlt] = 2;
        }
        else
        { // Send flag to true for each non-zero coefficient
          elm.component_mixing_type[iAlt] = 3;
          uint16_t iCmf                   = 0;
          elm.component_mixing_coefficient[iAlt][iCmf] =
            uint16_t(cvt.hatm.cgf[iAlt].cm.componentMixRed * Q_COMPONENT_MIXING_COEFFICIENT + 0.5f);
          iCmf++;
          elm.component_mixing_coefficient[iAlt][iCmf] = uint16_t(
            cvt.hatm.cgf[iAlt].cm.componentMixGreen * Q_COMPONENT_MIXING_COEFFICIENT + 0.5f);
          iCmf++;
          elm.component_mixing_coefficient[iAlt][iCmf] = uint16_t(
            cvt.hatm.cgf[iAlt].cm.componentMixBlue * Q_COMPONENT_MIXING_COEFFICIENT + 0.5f);
          iCmf++;
          elm.component_mixing_coefficient[iAlt][iCmf] =
            uint16_t(cvt.hatm.cgf[iAlt].cm.componentMixMax * Q_COMPONENT_MIXING_COEFFICIENT + 0.5f);
          iCmf++;
          elm.component_mixing_coefficient[iAlt][iCmf] =
            uint16_t(cvt.hatm.cgf[iAlt].cm.componentMixMin * Q_COMPONENT_MIXING_COEFFICIENT + 0.5f);
          iCmf++;
          elm.component_mixing_coefficient[iAlt][iCmf] = uint16_t(
            cvt.hatm.cgf[iAlt].cm.componentMixComponent * Q_COMPONENT_MIXING_COEFFICIENT + 0.5f);
          iCmf++;
          uint16_t sumCoefficients = 0;
          for(iCmf = 0; iCmf < MAX_NB_COMPONENT_MIXING_COEFFICIENT; iCmf++)
          {
            if(elm.component_mixing_coefficient[iAlt][iCmf] > 0 &&
               elm.component_mixing_coefficient[iAlt][iCmf] <= Q_COMPONENT_MIXING_COEFFICIENT)
            {
            ...
            else
            {
              logMsg(
                LOGLEVEL_ERROR,
                "component mixing coefficient for alternate %d color %d is greater than 1.0 (%d)",
                iAlt, iCmf,
                (float)elm.component_mixing_coefficient[iAlt][iCmf] /
                  Q_COMPONENT_MIXING_COEFFICIENT);
            }
            // Check if same mode as  alternate 0 and same coefficient, if not, then not common
            if(elm.component_mixing_type[0] == 3 && (elm.component_mixing_coefficient[0][iCmf] !=
                                                     elm.component_mixing_coefficient[iAlt][iCmf]))
            {
          ...
          if(sumCoefficients != Q_COMPONENT_MIXING_COEFFICIENT)
          {
            logMsg(LOGLEVEL_WARNING,
                   "Sum component mixing coefficient for alternate %d is not equal to 1.0, they "
                   "will be scaled to 1.0 at decoding",
                   iAlt);
          }
        }
        ...
        elm.gain_curve_num_control_points_minus_1[iAlt] =
          cvt.hatm.cgf[iAlt].gc.gainCurveNumControlPoints - 1;
        if(elm.gain_curve_num_control_points_minus_1[0] !=
           elm.gain_curve_num_control_points_minus_1[iAlt])
        {
        ...
        {
          elm.gain_curve_control_points_x[iAlt][iCps] = uint16_t(
            cvt.hatm.cgf[iAlt].gc.gainCurveControlPointX[iCps] * Q_GAIN_CURVE_CONTROL_POINT_X +
            0.5f);
          if(elm.gain_curve_control_points_x[0][iCps] !=
             elm.gain_curve_control_points_x[iAlt][iCps])
          {
            elm.has_common_curve_params_flag = false;
          }

          elm.gain_curve_control_points_y[iAlt][iCps] =
            uint16_t(std::abs(cvt.hatm.cgf[iAlt].gc.gainCurveControlPointY[iCps]) *
                       Q_GAIN_CURVE_CONTROL_POINT_Y +
                     0.5f);
        }
        ...
        if(!elm.gain_curve_use_pchip_slope_flag[iAlt])
        {
          for(uint16_t iCps = 0; iCps < cvt.hatm.cgf[iAlt].gc.gainCurveNumControlPoints; iCps++)
          {
            ...
            elm.gain_curve_control_points_theta[iAlt][iCps] = uint16_t(
              (theta + O_GAIN_CURVE_CONTROL_POINT_THETA) * Q_GAIN_CURVE_CONTROL_POINT_THETA + 0.5f);
          }
        }
      }
    }
...
void SMPTE_ST2094_50::writeSyntaxElementsToBinaryData()
{
  ...
  push_boolean(&payloadBinaryData, elm.has_custom_hdr_reference_white_flag,
               "has_custom_hdr_reference_white_flag", verboseLevel);
  ...
  if(elm.has_adaptive_tone_map_flag)
  {
    push_16bits(&payloadBinaryData, elm.baseline_hdr_headroom, "baseline_hdr_headroom",
                verboseLevel);
    push_boolean(&payloadBinaryData, elm.use_reference_white_tone_mapping_flag,
                 "use_reference_white_tone_mapping_flag", verboseLevel);
    if(!elm.use_reference_white_tone_mapping_flag)
    {
      push_bits(&payloadBinaryData, uint8_t(elm.num_alternate_images), 3, "num_alternate_images",
                verboseLevel);
      push_bits(&payloadBinaryData, uint8_t(elm.gain_application_space_chromaticities_mode), 2,
                "gain_application_space_chromaticities_mode", verboseLevel);
      push_boolean(&payloadBinaryData, elm.has_common_component_mix_params_flag,
                   "has_common_component_mix_params_flag", verboseLevel);
      push_boolean(&payloadBinaryData, elm.has_common_curve_params_flag,
                   "has_common_curve_params_flag", verboseLevel);

      if(elm.gain_application_space_chromaticities_mode == 3)
      {
        for(uint16_t iCh = 0; iCh < 8; iCh++)
        {
          push_16bits(&payloadBinaryData, elm.gain_application_space_chromaticities[iCh],
                      "gain_application_space_chromaticities[iCh]", verboseLevel);
        }
      }

      for(uint16_t iAlt = 0; iAlt < elm.num_alternate_images; iAlt++)
      {
        push_16bits(&payloadBinaryData, elm.alternate_hdr_headrooms[iAlt],
                    "alternate_hdr_headrooms[iAlt]", verboseLevel);
        // Write component mixing function parameters
        if(iAlt == 0 || !elm.has_common_component_mix_params_flag)
        {
          push_bits(&payloadBinaryData, uint8_t(elm.component_mixing_type[iAlt]), 2,
                    "component_mixing_type[iAlt]", verboseLevel);
          if(elm.component_mixing_type[iAlt] == 3)
          {
            ...
            {
              uint8_t flagValue =
                static_cast<uint8_t>(elm.has_component_mixing_coefficient_flag[iAlt][iCm]);
            ...
            for(uint8_t iCm = 0; iCm < MAX_NB_COMPONENT_MIXING_COEFFICIENT; iCm++)
            {
              ...
              {
                push_16bits(&payloadBinaryData, elm.component_mixing_coefficient[iAlt][iCm],
                            "component_mixing_coefficient[iAlt][iCm]", verboseLevel);
              }
            }
          }
          else
        ...
        {
          push_bits(&payloadBinaryData, elm.gain_curve_num_control_points_minus_1[iAlt], 5,
                    "gain_curve_num_control_points_minus_1[iAlt]", verboseLevel);
          push_boolean(&payloadBinaryData, elm.gain_curve_use_pchip_slope_flag[iAlt],
                       "gain_curve_use_pchip_slope_flag[iAlt]", verboseLevel);
          ...
          {
            push_16bits(&payloadBinaryData, elm.gain_curve_control_points_x[iAlt][iCps],
                        "gain_curve_control_points_x[iAlt][iCps]", verboseLevel);
          }
        }
        for(uint16_t iCps = 0; iCps < elm.gain_curve_num_control_points_minus_1[iAlt] + 1; iCps++)
        {
          push_16bits(&payloadBinaryData, elm.gain_curve_control_points_y[iAlt][iCps],
                      "gain_curve_control_points_y[iAlt][iCps]", verboseLevel);
        }
        if(!elm.gain_curve_use_pchip_slope_flag[iAlt])
        {
          for(uint16_t iCps = 0; iCps < elm.gain_curve_num_control_points_minus_1[iAlt] + 1; iCps++)
          {
            push_16bits(&payloadBinaryData, elm.gain_curve_control_points_theta[iAlt][iCps],
                        "gain_curve_control_points_theta[iAlt][iCps]", verboseLevel);
          }
        }
      }
    }
    else
    { // No more information need to be signaled when using Reference White Tone Mapping Operator
  ...
  else
  {
    ...
    logMsg(LOGLEVEL_DEBUG,
           "End SMPTE_ST2094_50::writeSyntaxElementsToBinaryData, payload size = %d bytes",
           payloadBinaryData.byteIdx);
...
void SMPTE_ST2094_50::decodeBinaryToSyntaxElements(std::vector<uint8_t> binary_data)
{
  ...
  elm.minimum_application_version =
    pull_bits(&payloadBinaryData, 3, "minimum_application_version", verboseLevel);
  pull_bits(&payloadBinaryData, 2, "zero_2bits", verboseLevel);

  elm.has_custom_hdr_reference_white_flag =
    pull_boolean(&payloadBinaryData, "has_custom_hdr_reference_white_flag", verboseLevel);
  elm.has_adaptive_tone_map_flag =
    pull_boolean(&payloadBinaryData, "has_adaptive_tone_map_flag", verboseLevel);
  ...
  if(elm.has_adaptive_tone_map_flag)
  {
    elm.baseline_hdr_headroom =
      pull_16bits(&payloadBinaryData, "baseline_hdr_headroom", verboseLevel);

    elm.use_reference_white_tone_mapping_flag =
      pull_boolean(&payloadBinaryData, "use_reference_white_tone_mapping_flag", verboseLevel);
    if(!elm.use_reference_white_tone_mapping_flag)
    {
      elm.num_alternate_images =
        pull_bits(&payloadBinaryData, 3, "num_alternate_images", verboseLevel);
      elm.gain_application_space_chromaticities_mode = pull_bits(
        &payloadBinaryData, 2, "gain_application_space_chromaticities_mode", verboseLevel);
      elm.has_common_component_mix_params_flag =
        pull_boolean(&payloadBinaryData, "has_common_component_mix_params_flag", verboseLevel);
      elm.has_common_curve_params_flag =
        pull_boolean(&payloadBinaryData, "has_common_curve_params_flag", verboseLevel);

      if(elm.gain_application_space_chromaticities_mode == 3)
      {
        for(uint16_t iCh = 0; iCh < 8; iCh++)
        {
          elm.gain_application_space_chromaticities[iCh] = pull_16bits(
            &payloadBinaryData, "gain_application_space_chromaticities[iCh]", verboseLevel);
        }
      }
      ...
      {
        elm.alternate_hdr_headrooms[iAlt] =
          pull_16bits(&payloadBinaryData, "alternate_hdr_headrooms[iAlt]", verboseLevel);

        // Read component mixing function parameters - Table C.4
        if(iAlt == 0 || !elm.has_common_component_mix_params_flag)
        {
          elm.component_mixing_type[iAlt] =
            pull_bits(&payloadBinaryData, 2, "component_mixing_type[iAlt]", verboseLevel);
          if(elm.component_mixing_type[iAlt] == 3)
          {
            uint8_t has_component_mixing_coefficient_flag = pull_bits(
              &payloadBinaryData, 6, "has_component_mixing_coefficient_flag[iAlt]", verboseLevel);
            // Decode the flags and the associated values
            for(uint8_t iCm = 0; iCm < MAX_NB_COMPONENT_MIXING_COEFFICIENT; iCm++)
            {
              elm.has_component_mixing_coefficient_flag[iAlt][iCm] =
                bool(has_component_mixing_coefficient_flag & (0x01 << (5 - iCm)));
              if(elm.has_component_mixing_coefficient_flag[iAlt][iCm])
              {
                elm.component_mixing_coefficient[iAlt][iCm] = pull_16bits(
                  &payloadBinaryData, "component_mixing_coefficient[iAlt][iCm]", verboseLevel);
              }
              else
              {
                elm.component_mixing_coefficient[iAlt][iCm] = 0;
              }
            }
          }
          else
        ...
        else
        {
          ...
          if(elm.component_mixing_type[0] == 3)
          {
            ...
            for(uint8_t iCm = 0; iCm < MAX_NB_COMPONENT_MIXING_COEFFICIENT; iCm++)
            {
              elm.has_component_mixing_coefficient_flag[iAlt][iCm] =
                elm.has_component_mixing_coefficient_flag[iAlt][iCm];
              elm.component_mixing_coefficient[iAlt][iCm] =
                elm.component_mixing_coefficient[0][iCm];
            }
          }
        }
        ...
        if(iAlt == 0 || !elm.has_common_curve_params_flag)
        {
          elm.gain_curve_num_control_points_minus_1[iAlt] = pull_bits(
            &payloadBinaryData, 5, "gain_curve_num_control_points_minus_1[iAlt]", verboseLevel);
          elm.gain_curve_use_pchip_slope_flag[iAlt] =
            pull_boolean(&payloadBinaryData, "gain_curve_use_pchip_slope_flag[iAlt]", verboseLevel);
          pull_bits(&payloadBinaryData, 2, "zero_2bits[iAlt]", verboseLevel);

          for(uint16_t iCps = 0; iCps < elm.gain_curve_num_control_points_minus_1[iAlt] + 1; iCps++)
          {
            elm.gain_curve_control_points_x[iAlt][iCps] = pull_16bits(
              &payloadBinaryData, "gain_curve_control_points_x[iAlt][iCps]", verboseLevel);
          }
        }
        else
        {
          elm.gain_curve_num_control_points_minus_1[iAlt] =
            elm.gain_curve_num_control_points_minus_1[0];
        ...
        for(uint16_t iCps = 0; iCps < elm.gain_curve_num_control_points_minus_1[iAlt] + 1; iCps++)
        {
          elm.gain_curve_control_points_y[iAlt][iCps] = pull_16bits(
            &payloadBinaryData, "gain_curve_control_points_y[iAlt][iCps]", verboseLevel);
        }
        if(!elm.gain_curve_use_pchip_slope_flag[iAlt])
        {
          for(uint16_t iCps = 0; iCps < elm.gain_curve_num_control_points_minus_1[iAlt] + 1; iCps++)
          {
            elm.gain_curve_control_points_theta[iAlt][iCps] = pull_16bits(
              &payloadBinaryData, "gain_curve_control_points_theta[iAlt][iCps]", verboseLevel);
          }
        }
      }
    }
    ...
    {
      pull_bits(&payloadBinaryData, 7, "zero_7bits", verboseLevel);
    }
  }
...
void SMPTE_ST2094_50::convertSyntaxElementsToMetadataItems()
{
  ...
  if(elm.has_adaptive_tone_map_flag)
  {
    ...
    if(elm.use_reference_white_tone_mapping_flag)
    {
      ...
      float headroom_to_anchor_ratio =
        std::min(cvt.hatm.baselineHdrHeadroom / log2(1000.0 / 203.0), 1.0);
    ...
    else
    {
      ...
      else if(elm.gain_application_space_chromaticities_mode == 3)
      {
        for(uint16_t iCh = 0; iCh < 8; iCh++)
        {
          cvt.hatm.gainApplicationSpaceChromaticities[iCh] =
            float(elm.gain_application_space_chromaticities[iCh]) /
            Q_GAIN_APPLICATION_SPACE_CHROMATICITY;
        }
      }
      else
      {
        logMsg(LOGLEVEL_ERROR, "gain_application_space_primaries=%d not defined",
               elm.gain_application_space_chromaticities_mode);
      }
      for(uint16_t iAlt = 0; iAlt < cvt.hatm.numAlternateImages; iAlt++)
      {
        cvt.hatm.alternateHdrHeadroom.push_back(float(elm.alternate_hdr_headrooms[iAlt]) /
                                                Q_HDR_HEADROOM);
        ...
        else if(elm.component_mixing_type[iAlt] == 3)
        {
          ...
          if(elm.component_mixing_type[iAlt] == 0)
          {
            cgf.cm.componentMixMax = 1.0f;
          }
          else if(elm.component_mixing_type[iAlt] == 1)
          {
            cgf.cm.componentMixComponent = 1.0f;
          }
          else if(elm.component_mixing_type[iAlt] == 2)
          {
          ...
          else if(elm.component_mixing_type[iAlt] == 3)
          {
            ...
            for(int k = 0; k < MAX_NB_COMPONENT_MIXING_COEFFICIENT; k++)
            {
              ...
              {
              case 0:
                cgf.cm.componentMixRed = value;
                break;
              case 1:
                cgf.cm.componentMixGreen = value;
                break;
              case 2:
                cgf.cm.componentMixBlue = value;
                break;
              case 3:
                cgf.cm.componentMixMax = value;
                break;
              case 4:
                cgf.cm.componentMixMin = value;
                break;
              case 5:
                cgf.cm.componentMixComponent = value;
                break;
              }
            }
          }
        ...
        {
          logMsg(LOGLEVEL_ERROR, "mix_encoding[%d]=%d not defined", iAlt,
                 elm.component_mixing_type[iAlt]);
        }
        ...
        for(uint16_t iCps = 0; iCps < cgf.gc.gainCurveNumControlPoints; iCps++)
        {
          cgf.gc.gainCurveControlPointX.push_back(
            float(elm.gain_curve_control_points_x[iAlt][iCps]) / Q_GAIN_CURVE_CONTROL_POINT_X);
          cgf.gc.gainCurveControlPointY.push_back(
            sign * float(elm.gain_curve_control_points_y[iAlt][iCps]) /
            Q_GAIN_CURVE_CONTROL_POINT_Y);
          if(!elm.gain_curve_use_pchip_slope_flag[iAlt])
          {
            hasSlopeParameter[iAlt] = true;
            float theta             = float(elm.gain_curve_control_points_theta[iAlt][iCps]) /
                            Q_GAIN_CURVE_CONTROL_POINT_THETA -
                          O_GAIN_CURVE_CONTROL_POINT_THETA;
  ...
  if(isHeadroomAdaptiveToneMap)
  {
    ...
    if(!isReferenceWhiteToneMapping)
    {
      ...
      if(cvt.hatm.numAlternateImages > 0)
      {
        ...
        for(size_t i = 0; i < cvt.hatm.cgf.size(); i++)
        { // Range-based for loop for convenience
