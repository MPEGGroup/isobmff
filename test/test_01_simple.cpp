...
TEST_CASE( "Check isobmff/01_simple.mp4" ) {
  ...
  SECTION( "Check Tracks" ) {
    ...
    err = ISOGetTrackReference( audioTrack, MP4_FOUR_CHAR_CODE('s','y','n','c'), 1, &outReferencedTrack );
    CHECK( err == MP4NoErr );
    ...
    err = ISOGetTrackReferenceCount(audioTrack, MP4_FOUR_CHAR_CODE('s','y','n','c'), &outReferenceIndex );
    CHECK( err == MP4NoErr );
  ...
  SECTION( "Check Media" ) {
    ...
    err = ISOGetIndMediaSample(audioMedia, 2, outSample, &outSize, &outDecodingTime, &outCTSOffset, &outDuration, &outSampleFlags, &outSampleDescIndex );
    CHECK( err == MP4NoErr );
    CHECK( outSize == 141 );
    ...
    err = ISOGetMediaSample(audioMedia, outSample, &outSize, 3000, &outDecodingTime, &outCompositionTime, &outDuration, outSampleDescription, &outSampleDescIndex, &outSampleFlags);
    CHECK( err == MP4NoErr );
